...
def analyze_column_dimensions(
    ...
    for idx, c in enumerate(columns):
        ...
        normalized.append({
            ...
            "dimension_key": (
                f"{year}|{scope}|{'RESTATED' if restated else 'ORIGINAL'}|"
                f"{measure}"
            ),
        })
    ...
    for year in sorted(repeated_years):
        ...
        missing_identity = [
            x for x in same_year if not x["scope"] and not x["measure"]
        ]
        identities = {(x["scope"], x["measure"]) for x in same_year}
        if missing_identity or len(identities) < len(same_year):
            issues.append({
                ...
                "detail": (
                    f"年份 {year} 出现多个逻辑列，但 scope/measure 组合不唯一；"
                    "可能存在父级表头或计量维度未绑定。"
                ),
            })
...
def capture_to_long_df(result: TableCaptureResult) -> pd.DataFrame:
    ...
    # Stable source-row identity is carried into Canonical Research Long.
    row_ids = {row.row_order: f"{result.pdf_sha256}:{result.table_query}:{row.row_order}" for row in result.rows}
    parent_row_ids: dict[int, str] = {}
    preceding_rows_by_label: dict[str, str] = {}
    for row in result.rows:
        parent_token = normalize_text(row.parent_section or "").rstrip("：:")
        if parent_token and parent_token in preceding_rows_by_label:
            parent_row_ids[row.row_order] = preceding_rows_by_label[parent_token]
        for source_label in (row.normalized_item, row.raw_item):
            label_token = normalize_text(source_label or "").rstrip("：:")
            if label_token:
                preceding_rows_by_label[label_token] = row_ids[row.row_order]
        ...
        for cell in row.cells:
            ...
            records.append({
                ...
                **block_dimensions(row),
                ...
                "data_year": col.year if col else None,
...
def capture_to_wide_df(result: TableCaptureResult) -> pd.DataFrame:
    ...
    """
    # v6.2: structure is a deterministic derived layer.  The immutable JSON
    # machine result remains untouched; audit fields travel with exported rows.
    long_df = infer_row_structure(capture_to_long_df(result))
