...
ROW_ROLES = {"SECTION", "DETAIL", "BREAKDOWN_DETAIL", "SUBTOTAL", "TOTAL", "IMPLICIT_TOTAL", "GROSS_VALUE", "NET_VALUE", "ADJUSTMENT", "MEMO_TEXT", "NOTE_TEXT"}

def infer_row_role(raw_item: str | None, row_type: str | None = None, *, has_value: bool = False) -> str:
    text = str(raw_item or "").strip()
    norm = re.sub(r"\s+", "", text)
    if not text and has_value: return "IMPLICIT_TOTAL"
...
def build_semantic_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i,row in enumerate(rows):
        ...
        role=infer_row_role(raw, copy.get("row_type"), has_value=copy.get("value") is not None)
        while parent_stack and parent_stack[-1][0] >= level: parent_stack.pop()
        parent=parent_stack[-1] if parent_stack else None
        row_id=copy.get("row_id") or "ROW_"+uuid.uuid4().hex
        copy.update({"row_id":row_id,"parent_row_id":parent[1] if parent else None,"row_role":role,"row_level":level,
