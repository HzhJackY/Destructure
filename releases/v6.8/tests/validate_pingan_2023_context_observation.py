...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="v66_pingan_context_") as tmp:
        ...
        value_columns = [c for c in wide.columns if c.startswith("COL_")]
        dimensions = pd.read_csv(merge["artifacts"]["column_dimensions"], encoding="utf-8-sig")
        assert value_columns and set(value_columns) == set(dimensions["column_id"]), value_columns
        assert {"company", "report_year", "data_year", "period_type", "currency_unit", "statement_scope", "restated_flag"}.issubset(dimensions.columns)
        assert not dimensions["data_year"].astype(str).str.endswith(".0").any(), dimensions.to_dict("records")
