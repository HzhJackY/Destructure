...
class AssetGovernanceRepository:
    ...
    def register_capture_version(
        ...
        with self.registry.connect() as conn:
            ...
            make_current = bool(certified and registration_status == "REGISTERED")
            if make_current:
                conn.execute(
            ...
            conn.execute(
                ...
                (
                    ...
                    "CERTIFIED_ACTIVE" if make_current else "ACTIVE", supersedes, None,
    ...
    def resolve_review(self, capture_id: str, status: str) -> None:
        with self.registry.connect() as conn:
            ...
            if action in {"CONFIRMED", "CONFIRMED_HUMAN", "CONFIRMED_AUTO", "CONFIRMED_OVERRIDE"}:
                ...
                conn.execute(
                    "UPDATE capture_versions SET review_status='UNRESOLVED',updated_at=? WHERE capture_id=?",
                    (_now(), capture_id),
                )
            ...
            conn.execute(
                """INSERT INTO asset_status_transitions(
                    transition_id,logical_asset_id,capture_id,previous_status,new_status,
                    actor,reason,evidence_json,source_ui_action,producer_version,created_at
                   ) VALUES(?,?,?,?,?,'USER','REVIEW_ADJUDICATION','{}','REVIEW_INBOX','v6.8',?)""",
    ...
    def search_assets(
        ...
        if not include_archived:
            ...
            where.append("cv.asset_status<>'ARCHIVED'")
        for field in (
    ...
    def bootstrap_existing_captures(self, producer_version: str) -> dict[str, int]:
        ...
        return {"logical_assets_created": assets_after-assets_before, "capture_versions_created": created_versions}
