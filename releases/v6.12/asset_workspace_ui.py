...
def render_asset_workspace(st, backend) -> None:
    ...
    routed_filters=dict(st.session_state.get("asset_workspace_filter") or {})
    if review_queue:
    ...
    for col,(field,label) in zip(fcols,filter_fields):
        options=["全部"]+facets.get(field,[])
        widget_key=f"asset_filter_{field}"
        routed_value=routed_filters.get(field)
        if (
            widget_key not in st.session_state
            and routed_value in options
        ):
            st.session_state[widget_key]=routed_value
        choice=col.selectbox(label,options,key=widget_key)
    ...
    with st.expander("资产级操作与保存视图"):
        def _keep_current_asset_focused() -> None:
            st.session_state["inspection_route"] = active_route.__dict__
            st.session_state["selected_logical_asset_id"] = selected_asset
            st.session_state["selected_capture_version_id"] = selected_version

        def _archive_current_logical_asset() -> None:
            backend.archive_service.archive([selected_asset],reason="LOGICAL_ASSET_WORKSPACE")
            # The default list hides archived assets. Keep this target visible so
            # the next explicit action cannot silently land on another asset.
            st.session_state["asset_include_archived"] = True
            _keep_current_asset_focused()

        def _restore_current_logical_asset() -> None:
            backend.archive_service.restore([selected_asset],reason="LOGICAL_ASSET_WORKSPACE")
            _keep_current_asset_focused()

        a,b=st.columns(2)
        a.button("归档当前逻辑资产",key="archive_selected_asset",on_click=_archive_current_logical_asset)
        b.button("恢复当前逻辑资产",key="restore_selected_asset",on_click=_restore_current_logical_asset)
