...
通常无需再次搬历史数据。""",
    ...
    if st.button(
        ...
        if report.get("rotated_old_backups"):
            st.info(f"已自动清理 {len(report['rotated_old_backups'])} 份过期备份。")
        st.json(report)
        st.session_state.pop("data_cleanup_preview", None)
        st.rerun()

    st.divider()
    st.subheader("备份管理与还原")
    backups = cleanup_svc.list_backups()
    if backups:
        df_bk = pd.DataFrame(backups)
        st.dataframe(
            df_bk[["stamp", "scope", "total_size_mb", "app_version"]],
            use_container_width=True,
            hide_index=True,
        )
        total_mb = sum(b["total_size_mb"] for b in backups)
        st.caption(f"共 {len(backups)} 份备份，合计 {total_mb:.1f} MB")

        selected_stamp = st.selectbox(
            "选择要还原的备份",
            [b["stamp"] for b in backups],
            key="restore_stamp_select",
        )
        restore_token = st.text_input(
            f"输入确认口令 {CLEANUP_CONFIRMATION_TOKEN}",
            type="password",
            key="restore_confirm_token",
        )
        if st.button(
            "从备份还原",
            type="primary",
            disabled=(restore_token != CLEANUP_CONFIRMATION_TOKEN),
            key="restore_from_backup_btn",
        ):
            with st.spinner("正在还原…"):
                result = cleanup_svc.restore_from_backup(
                    selected_stamp, confirmation=restore_token,
                )
            st.success("还原完成。")
            st.json(result)
            st.rerun()
