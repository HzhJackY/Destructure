...
def test_run_cleanup_clears_business_rows_keeps_config_and_backs_up(
    ...
    assert report["backup"]["database_size_bytes"] > 0
    # Archives are now zip files, not raw directories.
    archived_dir = backup_dir / "archived"
    assert archived_dir.is_dir()
    assert (archived_dir / "table_captures.zip").is_file()
    assert (archived_dir / "table_merges.zip").is_file()
    assert (archived_dir / "cache.zip").is_file()
    # Verify zip contents
    with zipfile.ZipFile(archived_dir / "table_captures.zip") as zf:
        assert "a.csv" in zf.namelist()
    with zipfile.ZipFile(archived_dir / "table_merges.zip") as zf:
        assert "b.csv" in zf.namelist()
    with zipfile.ZipFile(archived_dir / "cache.zip") as zf:
        assert "c.bin" in zf.namelist()
...
def test_run_cleanup_include_pdfs_clears_uploads_and_pdf_assets(
    ...
    backup_dir = Path(report["backup"]["database"]).parent
    archived_dir = backup_dir / "archived"
    assert (archived_dir / "uploads.zip").is_file()
    with zipfile.ZipFile(archived_dir / "uploads.zip") as zf:
        assert "d.pdf" in zf.namelist()
...
def test_capture_scope_clears_captures_but_keeps_certification(
    ...
    backup_dir = Path(report["backup"]["database"]).parent
    archived_dir = backup_dir / "archived"
    assert (archived_dir / "reviews.zip").is_file()
    with zipfile.ZipFile(archived_dir / "reviews.zip") as zf:
        assert "r.json" in zf.namelist()
...
def test_archive_dirs_creates_zip_with_integrity(tmp_path: Path):
    """archive_dirs must produce zip files that pass testzip()."""
    _, _, service = _service(tmp_path)
    backup_dir = tmp_path / "backup" / "test_zip"
    backup_dir.mkdir(parents=True)
    result = service.archive_dirs(backup_dir=backup_dir)
    archived = result["archived"]
    assert len(archived) > 0
    for key, info in archived.items():
        assert "error" not in info, f"{key} had error: {info.get('error')}"
        assert info["archive"].endswith(".zip")
        assert info["zip_sha256"]
        assert info["zip_size_bytes"] > 0
        with zipfile.ZipFile(info["archive"], "r") as zf:
            assert zf.testzip() is None


def test_archive_dirs_does_not_delete_on_failure(tmp_path: Path, monkeypatch):
    """If zip verification fails, source dir must NOT be deleted."""
    _, paths, service = _service(tmp_path)

    # Monkeypatch ZipFile.testzip to simulate integrity failure
    original_testzip = zipfile.ZipFile.testzip

    def _fake_testzip(self):
        return "CORRUPTED_ENTRY"

    monkeypatch.setattr(zipfile.ZipFile, "testzip", _fake_testzip)

    backup_dir = tmp_path / "backup" / "test_fail"
    backup_dir.mkdir(parents=True)
    result = service.archive_dirs(backup_dir=backup_dir)
    # Source dirs should still exist with their files
    assert (paths["table_captures"] / "a.csv").is_file()
    assert (paths["cache"] / "c.bin").is_file()
    for key, info in result["archived"].items():
        assert "error" in info
        assert "ZIP_INTEGRITY_FAILURE" in info["error"]


def test_list_backups_returns_sorted_metadata(tmp_path: Path):
    """list_backups must return backups newest-first with size info."""
    _, _, service = _service(tmp_path)
    # Create 3 backups
    for _ in range(3):
        service.run_cleanup(
        ...
        _seed_files(tmp_path)  # re-seed files for next cleanup
    backups = service.list_backups()
    assert len(backups) == 3
    # Newest first
    stamps = [b["stamp"] for b in backups]
    assert stamps == sorted(stamps, reverse=True)
    for bk in backups:
        assert bk["total_size_mb"] >= 0
        assert bk["scope"] in ("all", "capture")


def test_rotate_backups_keeps_max(tmp_path: Path):
    """rotate_backups must delete oldest backups exceeding max_backups."""
    registry = MetadataRegistry(tmp_path / "metadata.db")
    paths = _paths(tmp_path)
    _seed(registry)
    service = DataCleanupService(
        registry, paths, app_version="v6.12-test", max_backups=2,
    )
    # Create 4 backups
    for _ in range(4):
        _seed_files(tmp_path)
    ...
    # After the 4th cleanup, rotation should have kept only 2
    remaining = service.list_backups()
    assert len(remaining) == 2


def test_restore_from_backup_roundtrip(tmp_path: Path):
    """cleanup -> restore must recover DB rows and archived files."""
    registry, paths, service = _service(tmp_path)
    # Check counts before cleanup
    with registry.connect() as conn:
        occ_before = conn.execute(
            "SELECT COUNT(*) FROM statement_occurrences"
        ).fetchone()[0]
    assert occ_before == 1
    ...
    stamp = report["backup"]["stamp"]

    # DB should be empty after cleanup
    with registry.connect() as conn:
        assert conn.execute(
            "SELECT COUNT(*) FROM statement_occurrences"
        ).fetchone()[0] == 0
    ...
    result = service.restore_from_backup(
        stamp, confirmation=CLEANUP_CONFIRMATION_TOKEN,
    )
    assert result["restored"]["database"]["sha256_verified"] is True

    # DB rows should be restored
    registry2 = MetadataRegistry(tmp_path / "metadata.db")
    with registry2.connect() as conn:
        assert conn.execute(
            "SELECT COUNT(*) FROM statement_occurrences"
        ).fetchone()[0] == 1

    # Archived files should be extracted
    assert (paths["table_captures"] / "a.csv").is_file()


def test_restore_requires_confirmation(tmp_path: Path):
    """restore_from_backup must reject wrong confirmation tokens."""
    _, _, service = _service(tmp_path)
    report = service.run_cleanup(
        confirmation=CLEANUP_CONFIRMATION_TOKEN,
        include_pdfs=False,
    ...
    stamp = report["backup"]["stamp"]
    with pytest.raises(PermissionError, match="RESTORE_CONFIRMATION_REQUIRED"):
        service.restore_from_backup(stamp, confirmation="WRONG")


def test_restore_rejects_missing_backup(tmp_path: Path):
    _, _, service = _service(tmp_path)
    with pytest.raises(FileNotFoundError, match="BACKUP_NOT_FOUND"):
