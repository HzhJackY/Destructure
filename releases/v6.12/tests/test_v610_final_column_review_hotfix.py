...
def test_sparse_anchored_cell_does_not_shift_into_first_period():
    result={
        "columns":[{"year":"2025"},{"year":"2024"}],
        "rows":[
            {"row_order":1,"row_type":"DETAIL","cells":[
                {"column_ordinal":1,"raw":"9,688","parsed_number":9688.0},
            ]},
            {"row_order":2,"row_type":"TOTAL","cells":[
                {"column_ordinal":0,"raw":"100","parsed_number":100.0},
                {"column_ordinal":1,"raw":"90","parsed_number":90.0},
            ]},
        ],
    }
    review=review_final_data_columns(result)
    codes={issue["reason_code"] for issue in review["issues"]}
    assert "VALUE_COLUMN_COUNT_MISMATCH" not in codes
    assert review["last_column_check"]["rows_with_last_token"] == 2
