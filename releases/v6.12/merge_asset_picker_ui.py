...
CLASSIFICATION_AXIS_DISPLAY_NAMES = {
    "ASSET_TYPE": "按资产类型",
...
def capture_block_display(record: Mapping[str, Any]) -> str:
    """Return a readable block title without changing stable Capture identity."""
    axis = str(record.get("classification_axis") or "").strip().upper()
    if axis in CLASSIFICATION_AXIS_DISPLAY_NAMES:
        return CLASSIFICATION_AXIS_DISPLAY_NAMES[axis]
    title = str(
        record.get("table_query")
        or record.get("source_table_title")
        or record.get("block_title")
        or ""
    ).strip()
    return title if any(char.isalnum() for char in title) else ""
...
def merge_asset_label(record: Mapping[str, Any]) -> str:
    ...
    block_title = capture_block_display(record)
    if block_title and block_title not in parts:
        parts.append(block_title)
    display_name = str(record.get("display_name") or "").strip()
    if (
        display_name
        and any(char.isalnum() for char in display_name)
        and display_name not in parts
    ):
...
def merge_project_label(record: Mapping[str, Any]) -> str:
    """Build a dropdown label that never renders a missing name as ``None``."""
    run_id = _text(record.get("run_id") or record.get("merge_id"), "")
    display_name = _text(record.get("display_name"), run_id or "未命名合表项目")
    if run_id and display_name != run_id:
