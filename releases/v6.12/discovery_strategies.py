...
def _certified_candidate_bbox(
    ...
    if isinstance(raw_bbox, dict) and all(
        key in raw_bbox for key in ("x0", "y0", "x1", "y1")
    ):
        return tuple(float(raw_bbox[key]) for key in ("x0", "y0", "x1", "y1"))

    evidence = dict(candidate.get("evidence") or {})
    logical_bbox = dict(evidence.get("logical_table_bbox") or {})
    for page_entry in logical_bbox.get("pages") or []:
        if int(page_entry.get("page") or 0) != int(start_page):
            continue
        page_bbox = dict(page_entry.get("bbox") or {})
        if all(key in page_bbox for key in ("x0", "y0", "x1", "y1")):
            return tuple(
                float(page_bbox[key]) for key in ("x0", "y0", "x1", "y1")
            )

    for segment in candidate.get("certified_segments") or []:
        if int(segment.get("start_page") or 0) != int(start_page):
            continue
        segment_bbox = dict(segment.get("bbox") or {})
        if all(key in segment_bbox for key in ("x0", "y0", "x1", "y1")):
            return tuple(
                float(segment_bbox[key]) for key in ("x0", "y0", "x1", "y1")
            )
    return None
...
class CertifiedTargetStrategy(BaseStrategy):
    ...
    def resolve_target(self, candidate: dict[str, Any], context: dict[str, Any]) -> ResolvedCaptureTarget:
        ...
        title = literal_capture_query_title(
            candidate.get("target_heading")
            or candidate.get("capture_query_title")
            or ""
        )
        return ResolvedCaptureTarget.new(
            ...
            bbox=_certified_candidate_bbox(candidate, page),
            title=title,
