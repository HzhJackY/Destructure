...
def compose_note_reference(header: str | None, row_value: str | int | None) -> dict[str, str]:
    """Compose ``附注八-9`` from a header and a row cell without guessing.

    Empty row values remain explicit ABSENT_ON_STATEMENT.  A numeric row cell is
    treated as a reference only if the column header contains 附注/注.
    """
...
def build_capture_plan(occurrence: StatementOccurrence, *, table_family: str | None = None,
                       certified_ids: Iterable[str] = ()) -> dict[str, Any]:
    ...
    for order, child in enumerate(occurrence.child_rows, start=1):
        target_page = child.get("confirmed_note_pdf_page_index") or child.get("candidate_note_pdf_page_index") or child.get("candidate_note_page")
        items.append({
            ...
            "candidate_note_pdf_page_index": target_page,
            "confirmed_note_pdf_page_index": child.get("confirmed_note_pdf_page_index"),
            "status": "READY" if target_page else "REVIEW_REQUIRED",
