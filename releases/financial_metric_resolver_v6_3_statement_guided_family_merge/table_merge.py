...
def apply_mapping(raw_long: pd.DataFrame, mapping_queue: pd.DataFrame) -> pd.DataFrame:
    ...
    def key_for(row) -> str:
        ...
        return f"{prefix}::{row.get('table_id')}::{section}::{item}::{row_path}"
...
def materialize_canonical(
    ...
    for key, g in numeric.groupby(dims, dropna=False, sort=False):
        ...
        missing_dimensions = (_dimension_missing(key[dims.index("year")]) if "year" in dims else False) or (_dimension_missing(key[dims.index("scope")]) if "scope" in dims else False)
        ...
        base["conflict_severity"] = ("WARNING" if conflict_reasons and set(conflict_reasons) == {"REVIEW_REQUIRED_DIMENSION_AMBIGUITY"} else ("BLOCKING" if conflict_reasons else "OK"))
