...
class DeepSeekProvider(LLMProvider):
    ...
    def select_candidate(
        ...
        system = (
            ...
            "数值由确定性程序从选中候选及其恢复证据中提取，你只负责选择最匹配的候选。"
            "重要财务语义规则：在同一现金流活动类别内，"
            "“产生的现金流量净额”与“使用的现金流量净额”通常是同一canonical净现金流指标的方向性措辞，"
            "若经营/投资/筹资类别一致且都是现金流量净额，不得仅因“产生/使用”措辞不同而abstain。"
            "不得自行改变数值正负号；符号由确定性程序处理。"
            "候选has_values=false并不代表科目错误，它可能是PDF标签/数值拆行；"
            "若recovery_evidence存在，应该结合恢复证据判断。"
            "当多个候选语义同样匹配时，必须优先 evidence_quality 更高、证据更完整的候选："
            "有数值、有期间/表头、有与指标类型兼容的单位、结构化表来源，优先于缺单位/缺期间/恢复链候选。"
            "证据不足、活动类别不同或指标定义实质不同才必须 abstain。"
...
class GeminiProvider(LLMProvider):
    ...
    def select_candidate(
        ...
        prompt = (
            ...
            "数值由确定性程序从选中候选及恢复证据提取，你只负责选择候选；证据不足必须 abstain。\n"
            "财务语义规则：同一经营/投资/筹资活动类别下，“产生的现金流量净额”和“使用的现金流量净额”"
            "通常属于同一canonical净现金流指标，不得只因产生/使用措辞不同而abstain。"
            "不得自行修改数值符号。has_values=false可能是PDF拆行；有recovery_evidence时应结合判断。"
            "多个候选语义相同时，优先 evidence_quality 更高且单位/期间/结构证据更完整的候选。\n"
            f"用户输入: {user_metric}\n"
