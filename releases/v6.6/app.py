...
被截断的后续数据保存在 boundary_excluded_rows.csv。""",
    ...
    all_capture_records = BACKEND.asset_service.list_captures(
        lifecycle_status="ACTIVE", limit=100000,
    )
    ...
    with tabs[0]:
        ...
        df = pd.read_csv(path) if path.exists() else pd.DataFrame()
        value_columns = [str(column) for column in df.columns if str(column).startswith("company=")]
        current_observation_schema = (
            manifest.get("canonical_observation_schema_version")
            == "6.6_EXPLICIT_OBSERVATION_DIMENSIONS"
            and bool(value_columns)
            and all("report_year=" in column and "data_year=" in column for column in value_columns)
        )
        if not current_observation_schema:
            st.warning(
                "此合表项目的宽表是旧版派生产物，不能作为 v6.6 Canonical 宽表解释。"
                "旧列头可能把公司、报告年、数据年、单位和重述状态混成一段文本。"
            )
            st.caption(
                "可在不修改源 Capture 或人工映射的前提下重新生成派生产物。"
                "注意：若源 Capture 在旧版已把百万元错误写为元，必须重抓原 PDF；"
                "重新合表不会猜测或修正历史金额。"
            )
            if st.button("按当前 v6.6 观察值契约重新生成此合表", key=f"refresh_v66_observation_{merge_name}"):
                try:
                    refresh_merge_project(merge_dir)
                    ensure_merge_metadata(merge_dir)
                    st.success("已按当前 v6.6 观察值契约重建派生合表输出。")
                    st.rerun()
                except Exception as exc:
                    st.exception(exc)
            ...
            df = pd.DataFrame()
        st.caption(
            "当前 Canonical 宽表列使用具名观察维度：company、report_year、data_year、"
            "period_type、currency_unit、statement_scope、restated_flag。"
