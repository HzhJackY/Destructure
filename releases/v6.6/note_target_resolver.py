...
def parse_note_reference(value: str) -> tuple[str, int | None]:
    ...
    match = re.search(r"(?:附注|注)?([一二三四五六七八九十\d]+)[-、.．]?([一二三四五六七八九十\d]+)$", compact)
    if not match:
        return "", None
    return match.group(1), chinese_ordinal(match.group(2))
...
class NoteReferenceResolver:
        ...
        for index, page in enumerate(pages, 1):
            ...
            for pos in range(len(lines)):
                ...
                if re.search(r"信用风险|减值测试|相关风险", block) and not table_signature:
                    score -= 0.35
                method = "SECTION_ORDINAL_SEMANTIC" if page_section_match and ordinal_exact and semantic else ("SECTION_SEMANTIC" if page_section_match and semantic else "SEMANTIC_FALLBACK")
    ...
    def certify(self, candidate: dict[str, Any], *, actor: str = "USER") -> dict[str, Any]:
        ...
        return {"status": "CERTIFIED_NOTE_TARGET", "confirmed_note_pdf_page_index": candidate["pdf_page_index"],
                "target_heading": candidate.get("heading"), "locator_method": candidate.get("locator_method"),
