...
class TableCaptureRunner:
    ...
    def _run_one(self,job:dict)->None:
        ...
        try:
            ...
            result=self.capture_callable(pdf_path=Path(payload["pdf_path"]),table_query=payload["table_query"],note_number=options.get("note_number"),start_page_override=options.get("start_page_override"),max_pages=int(options.get("max_pages",8)),header_parser_mode=options.get("header_parser_mode","AUTO"),batch_id=job.get("batch_id"),progress_callback=None, guided_target_required=bool(options.get("guided_target_required")), certified_note_target=options.get("certified_note_target"), table_family=family.get("display_name"), member_table=options.get("member_table") or payload.get("table_query"), member_table_role=options.get("member_table_role") or payload.get("target_role"), source_table_title=options.get("source_table_title") or payload.get("table_query"), note_reference=options.get("note_reference") or options.get("note_number"), member_table_order=options.get("member_table_order"))
            evidence=result.get("result") or {}
            from capture_library import capture_readiness
            readiness=capture_readiness(evidence)
            review=not bool(readiness["merge_ready"])
