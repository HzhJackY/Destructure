...
def _extract_tail_reference(line: str, label: str, note_header: str) -> dict[str, str]:
    ...
    return compose_note_reference(note_header, item.group(1) if item else "")
...
def discover(pdf_path, cache_root, *, display_name: str, company: str = "", report_year: str = "",
             filing_type: str = "ANNUAL_REPORT", preset_name: str | None = None, text_provider=None) -> list[dict[str, Any]]:
    ...
    for statement_type in candidate_types:
        ...
        for rec in index:
            if rec.page_number not in pages:
                continue
            note_header = ""
            for raw_line in rec.text.splitlines():
                text = " ".join(raw_line.split())
                if re.search(r"(?:附注|注)\s*[一二三四五六七八九十百\d]+", text):
                    note_header = text
                matched = next((t for t in sorted(tokens, key=len, reverse=True) if t in text), None)
                if not matched:
                    continue
                note = _extract_tail_reference(text, matched, note_header)
