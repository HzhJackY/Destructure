...
class V65StatementAnchoredFamilyTests(unittest.TestCase):
    ...
    def test_NOTE_REFERENCE_NULL_LOCATOR_FALLBACK_PASS(self):
        ...
        plan = build_capture_plan(finance_occurrence(refs=False))
    ...
    def test_STATEMENT_TO_NOTE_GRAPH_CAPTURE_PLAN_PASS(self):
        plan = build_capture_plan(finance_occurrence())
    ...
    def test_GUIDED_CAPTURE_NO_REDUNDANT_TARGET_SELECTION_PASS(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            plan = build_capture_plan(finance_occurrence()); plan["plan_id"] = "PLAN_TEST"
    ...
    def test_NO_FINANCIAL_VALUE_INVENTION_PASS(self):
        self.assertNotIn("value", build_capture_plan(finance_occurrence())["items"][1])
    ...
    def test_STATEMENT_NOTE_RECONCILIATION_ROUNDING_PASS(self):
        ...
        self.assertEqual(build_capture_plan(occurrence)["anchor"]["rows"][1]["value"], 900)

    def test_VERSION_SINGLE_SOURCE_V65_PASS(self):
        root = Path(__file__).resolve().parents[1]
        self.assertIn('APP_VERSION = "v6.5.1"', (root / "version.py").read_text(encoding="utf-8"))
