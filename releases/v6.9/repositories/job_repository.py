...
class JobRepository:
    ...
    def _row(r):
        ...
        return d
    def list(self,*,batch_id:Optional[str]=None,status:Optional[str]=None,job_type:Optional[str]=None,limit:int=500)->list[dict[str,Any]]:
        where=[];params=[]
        if batch_id:where.append('batch_id=?');params.append(str(batch_id))
        if status:where.append('status=?');params.append(str(status))
        if job_type:where.append('job_type=?');params.append(str(job_type))
