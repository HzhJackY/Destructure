...
def rank_and_preselect(candidates:Iterable[dict[str,Any]],context:dict[str,Any]|None=None,
    ...
    for (source,scope),rows in by_scope.items():
        ...
        choose=bool(scope in allowed and top and top["total_score"]>=float(cfg["min_score"]) and margin>=float(cfg["margin"]))
        if choose and int(cfg["max_per_scope"])>0:
            ...
            decisions[f"{source}::{scope}"]={"status":"SINGLE_PRESELECTED","candidate_id":top["occurrence_id"],"margin":margin}
        else:
