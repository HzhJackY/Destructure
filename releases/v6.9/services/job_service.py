...
class JobService:
    ...
    def batch_summaries(self, *, job_type: str = "TABLE_CAPTURE", limit: int = 100):
        grouped: dict[tuple[str, str], dict[str, Any]] = {}
        for row in self.repo.list(job_type=job_type, limit=100000):
            key = (str(row.get("batch_id") or ""), str(row.get("status") or "UNKNOWN"))
            item = grouped.setdefault(
                key,
                {"batch_id": key[0], "status": key[1], "count": 0, "updated_at": ""},
            )
            item["count"] += 1
            item["updated_at"] = max(
                str(item.get("updated_at") or ""), str(row.get("updated_at") or "")
            )
        return sorted(
            grouped.values(), key=lambda item: item["updated_at"], reverse=True
        )[: max(1, int(limit))]
