...
TABS=["概览","PDF 证据","附注容器与表块","表头拓扑","行结构","最终数据复核","Canonical 数据","勾稽与质量","审核","版本对比","使用情况"]
...
def render_capture_inspection_panel(
    ...
    if tab=="概览":
        ...
        review_summary=backend.review_task_service.materialize(capture_version_id)
        ...
        st.dataframe([{
            "row_id":row.get("row_order"),"parent":row.get("parent_section"),
            "row_path":f"{row.get('parent_section') or ''}/{row.get('normalized_item') or ''}",
            "level":row.get("row_level"),"type":row.get("row_type"),"role":row.get("row_role"),
            "raw_item":row.get("raw_item"),"normalized_item":row.get("normalized_item"),
            "derivation":row.get("derivation_method") or row.get("label_derivation"),
            "derived_from":row.get("derived_from_rows"),
        } for row in rows],use_container_width=True,hide_index=True)
        editable=pd.DataFrame([{
            "row_id":row.get("row_order"),"parent":row.get("parent_section"),
            "row_level":row.get("row_level"),"row_role":row.get("row_role") or row.get("row_type"),
            "normalized_item":row.get("normalized_item"),
        } for row in rows])
        edited_rows=st.data_editor(editable,use_container_width=True,hide_index=True,key=f"row_editor_{capture_version_id}")
        ...
        final=backend.review_task_service.materialize(capture_version_id)["final_data_review"]
        last=final["last_column_check"]
        (st.success if last["status"]=="PASS" else st.error)(
            f"最后一列专项检查：{last['status']}；"
            f"{last['rows_with_last_token']}/{last['row_count']} 行具有末列 token"
        )
        ...
        final_actions=st.columns(2)
        if final_actions[0].button("确认最终数据列",disabled=readonly_mode or bool(final["issues"]),key=f"confirm_final_data_{capture_version_id}"):
            backend.review_service.decide_task(capture_id=capture_version_id,task_type="FINAL_DATA_COLUMN_REVIEW",decision="CONFIRMED")
            st.rerun()
        if final_actions[1].button("标记未解决",disabled=readonly_mode,key=f"unresolved_final_data_{capture_version_id}"):
