...
def _sync_capture_once(run_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        readiness={
            ...
            'semantic_status':meta.get('semantic_status') or 'REVIEW_REQUIRED',
            'capture_quality_status':meta.get('capture_quality_status') or 'REVIEW_REQUIRED',
            'mixed_cell_count':meta.get('mixed_cell_count'),
            'unresolved_implicit_rows':meta.get('unresolved_implicit_rows'),
            'merge_blockers':meta.get('merge_blockers') or ['READINESS_EVALUATION_FAILED'],
            'merge_ready':bool(meta.get('merge_ready')),
        }
    meta.update({
