...
def _infer_statement_scope(text: str, statement_type: str, page_number: int = 0, statement_pages: list[int] | None = None) -> str:
    compact_top = normalize_text("\n".join(str(text or "").splitlines()[:25]))
    full_compact = normalize_text(text or "")
    titles = {
        "BALANCE_SHEET": "资产负债表",
    ...
    # 1. Direct title matching in top lines
    if title and (f"合并{title}" in compact_top or f"合并及公司{title}" in compact_top or ("合并" in compact_top and title in compact_top)):
        return "CONSOLIDATED"
    if title and ("公司资产负债表" in compact_top or "母公司" in compact_top or ("本公司" in compact_top and "合并" not in compact_top)):
        return "PARENT_COMPANY"

    # 2. Token matching in full page text
    if "合并" in full_compact or "本集团" in full_compact:
        return "CONSOLIDATED"
    if "公司资产负债表" in full_compact or "母公司资产负债表" in full_compact or "本公司" in full_compact:
        return "PARENT_COMPANY"

    # 3. Sequence fallback in formal financial report chapter (pages > 30): 1st BS page = CONSOLIDATED, 2nd BS page = PARENT_COMPANY
    if statement_pages and page_number in statement_pages:
        formal_pages = [p for p in statement_pages if p > 30]
        if formal_pages:
            if page_number == formal_pages[0]:
                return "CONSOLIDATED"
            elif len(formal_pages) > 1 and page_number == formal_pages[1]:
                return "PARENT_COMPANY"
