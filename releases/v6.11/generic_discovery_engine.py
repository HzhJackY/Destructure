...
class GenericDiscoveryService:
    ...
    def _statement_strategy(self, pdf_path: Path, family: dict[str, Any], members: list[dict[str, Any]], company: str, report_year: str, filing_type: str) -> list[dict[str, Any]]:
        ...
        # Stage A must use the same OCR-aware physical page source as the
        # certified offline statement resolver.  A native-text-only index can
        # make an early electronic summary page appear to be the sole anchor
        # while image-only audited statements are invisible to the GUI.
        #
        # Keep an explicit, auditable native fallback for environments where
        # OCR is unavailable; never silently suppress the failure or pretend
        # that the fallback saw every physical page.
        try:
            from document_index_profile import fast_index_profile_kwargs
            fast_records, fast_index_meta = build_fast_index(
                Path(pdf_path), self.cache_root, **fast_index_profile_kwargs()
            )
            index = fast_index_to_text_index(fast_records)
            audit["statement_index_source"] = "FAST_INDEX_OCR_AWARE"
