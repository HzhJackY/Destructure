...
def test_descendant_detection() -> None:
    ...
    # A visual statement row may expand into many extracted text lines; a
    # large raw distance remains inside the parent until a real outside-family
    # row is observed.
    assert _is_descendant_of_parent(25, 3, list(range(30)))
    assert not _is_descendant_of_parent(
        25,
        3,
        structural_boundary_index=20,
    )
