...
from pdf_selection_workspace import company_options, filter_pdfs, infer_pdf_dimensions
...
class V65StatementAnchoredFamilyTests(unittest.TestCase):
    ...
    def test_COMPANY_IDENTITY_YEAR_ANNUAL_REPORT_SUFFIX_PASS(self):
        actual = infer_pdf_dimensions(Path("5ea1048c3a93_中国人寿2023年年度报告.pdf"))
        self.assertEqual(actual, {"company": "中国人寿", "year": "2023"})
    ...
    def test_GUIDED_CAPTURE_NO_REDUNDANT_TARGET_SELECTION_PASS(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            result = GuidedCaptureService(registry=registry, runner=runner, audit_dir=root).execute(
                plan,pdf_path=root / "input.pdf",options={
                    "capture_scope_policy":"ALL_NOTE_TABLES",
                    "selected_block_roles":["SUPPLEMENTARY_TABLE"],
                    "selected_block_ids":["BLOCK_TEST"],
                },
            )
            self.assertEqual(len(runner.enqueued), 4)
            request = runner.enqueued[0]["request"]
            self.assertEqual(request.capture_scope_policy,"ALL_NOTE_TABLES")
            self.assertEqual(request.selected_block_roles,("SUPPLEMENTARY_TABLE",))
            self.assertEqual(request.selected_block_ids,("BLOCK_TEST",))
