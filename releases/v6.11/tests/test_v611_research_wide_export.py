...
def test_build_research_wide_frame_keeps_only_identity_unit_and_data():
    ...
    assert list(trimmed.columns) == [
        "member_table", "canonical_item", "unit", "COL_00001", "COL_00002",
    ]
    ...
    assert trimmed.iloc[0]["unit"] == "百万元"
    assert RESEARCH_WIDE_FIXED_COLUMNS == (
        "member_table", "canonical_item", "unit",
    )
...
def test_write_presentation_wide_sheet_accepts_custom_sheet_name(tmp_path):
    ...
    assert ws.cell(row=2, column=4).alignment.horizontal == "center"
    assert ws.cell(row=2, column=4).border.left.style == "medium"
    assert ws.cell(row=2, column=4).border.right.style == "medium"
    ...
    assert ws.cell(row=data_row, column=4).value == 100
...
def test_write_presentation_wide_sheet_bolds_total_rows_with_light_fill(tmp_path):
    ...
    research = pd.DataFrame([
        {"member_table": "债权投资", "canonical_item": "政府债", "unit": "百万元",
         "COL_00001": 100, "COL_00002": 200},
        {"member_table": "债权投资", "canonical_item": "小计", "unit": "百万元",
         "COL_00001": 150, "COL_00002": 250},
        {"member_table": "债权投资", "canonical_item": "合计", "unit": "百万元",
         "COL_00001": 150, "COL_00002": 250},
    ])
    ...
    assert ws.cell(row=detail_row, column=4).font.bold is False
    assert ws.cell(row=detail_row, column=4).fill.patternType is None
    for row in (subtotal_row, total_row):
        assert ws.cell(row=row, column=2).font.bold is True
        assert ws.cell(row=row, column=4).font.bold is True
        assert ws.cell(row=row, column=2).fill.patternType == "solid"
        assert ws.cell(row=row, column=4).fill.fgColor.rgb == "00FFF2CC"
...
def test_write_merge_outputs_emits_research_wide_artifacts(tmp_path):
    ...
    assert research_csv["unit"].tolist() == ["百万元"]
    ...
    assert ws.cell(row=2, column=1).value == "附注表名"
    first_dimension = ws.cell(row=2, column=4).value
...
def test_write_merge_outputs_bolds_total_rows_in_research_wide_xlsx(tmp_path):
    ...
    assert ws.cell(row=data_row, column=4).font.bold is True
    assert ws.cell(row=data_row, column=4).fill.patternType == "solid"
    assert ws.cell(row=data_row, column=4).fill.fgColor.rgb == "00FFF2CC"
...
def test_research_wide_report_year_groups_are_centered_and_bordered(tmp_path):
    ...
    research = pd.DataFrame([{
        "member_table": "债权投资", "canonical_item": "债券", "unit": "千元",
    ...
    assert ws["D2"].value == "2023年报"
    assert ws["E2"].value == "2024年报"
    assert ws["G2"].value == "2025年报"
    assert "E2:F2" in merged_ranges
    assert ws["D3"].value == "2023"
    assert ws["E3"].value == "2023"
    assert "D3:E3" not in merged_ranges
    assert ws["E3"].border.right.style == "medium"
    assert ws["F3"].border.left.style == "medium"
    assert ws["F3"].border.right.style == "medium"
    assert ws["G3"].border.left.style == "medium"
    assert ws["D2"].alignment.horizontal == "center"
    assert ws["D2"].border.left.style == "medium"
    assert ws["D2"].border.right.style == "medium"
    assert ws["E2"].border.left.style == "medium"
    assert ws["F2"].border.right.style == "medium"
    assert ws["G2"].border.left.style == "medium"
    assert ws["G2"].border.right.style == "medium"
