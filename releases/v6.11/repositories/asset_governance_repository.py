...
class AssetGovernanceRepository:
    ...
    def identity_payload(metadata: dict[str, Any]) -> dict[str, str]:
        ...
        certified_target = (
            metadata.get("certified_target")
            if isinstance(metadata.get("certified_target"),dict)
            else {}
        )
        certified_logical_table_id = str(
            metadata.get("certified_logical_table_id")
            or certified_target.get("logical_table_id")
            or ""
        ).strip()
        if certified_logical_table_id:
            identity["certified_logical_table_id"] = (
    ...
    def get_or_create_logical_asset(self, metadata: dict[str, Any]) -> dict[str, Any]:
        with self.registry.connect() as conn:
            row = self._get_or_create_logical_asset_in_tx(conn, metadata)
        return dict(row)

    def _get_or_create_logical_asset_in_tx(
        self, conn, metadata: dict[str, Any],
    ):
        """Get/create a Logical Asset in the caller's transaction."""
    ...
    def register_capture_version(
        ...
        review_status: str, certified: bool, asset_status: str | None = None,
    ) -> dict[str, Any]:
        with self.registry.connect() as conn:
            row = self._register_capture_version_in_tx(
                conn,
    ...
    def _register_capture_version_in_tx(
        self, conn, *, logical_asset_id: str, capture_id: str,
        producer_version: str, processing_status: str,
        registration_status: str, quality_status: str,
        review_status: str, certified: bool,
        asset_status: str | None = None,
    ):
        """Register/update one Capture Version in the caller's transaction."""
        effective_asset_status = str(
            asset_status or ("CERTIFIED_ACTIVE" if certified else "ACTIVE")
        )
        existing = conn.execute(
        ...
        if existing:
            conn.execute(
                """UPDATE capture_versions
                   SET processing_status=?,registration_status=?,quality_status=?,
                       review_status=?,asset_status=?,updated_at=?
                   WHERE capture_id=?""",
                (
                    processing_status,registration_status,quality_status,
                    review_status,effective_asset_status,_now(),capture_id,
                ),
            )
            return conn.execute(
                "SELECT * FROM capture_versions WHERE capture_id=?", (capture_id,)
        ...
        # "Current" means the latest registered working Capture Version.
        ...
        conn.execute(
            ...
            (
                logical_asset_id, capture_id, number, int(make_current),
                processing_status, registration_status, quality_status,
                review_status, effective_asset_status, supersedes, None,
        ...
        with self.registry.connect() as conn:
            ...
            children = [
                dict(row) for row in conn.execute(
                    """SELECT cbc.*,tb.block_title,tb.block_role,
                              tb.classification_axis,tb.block_terminal_type,tb.start_pdf_page,
