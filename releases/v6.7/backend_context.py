...
from services import AssetService,BatchService,MergeService,PdfService,JobService,CaptureService,ReviewService,RegistryService,DiscoveryService,GuidedCaptureService,ResearchBatchService
...
def build_backend_services(paths: dict[str, Path]) -> BackendServices:
    ...
    guided_capture_service=GuidedCaptureService(registry=registry,runner=runner,audit_dir=paths['table_captures'])
    ...
    return BackendServices(registry,synchronizer,capture_repo,batch_repo,merge_repo,pdf_repo,job_repo,asset,batch,merge,pdf,job,capture,review,registry_service,runner,discovery,discovery_service,guided_capture_service,research_batch_service,research_definition_service,generic_discovery_service)
