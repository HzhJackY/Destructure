...
class GenericDiscoveryService:
    ...
    def _statement_strategy(self, pdf_path: Path, family: dict[str, Any], members: list[dict[str, Any]], company: str, report_year: str, filing_type: str) -> list[dict[str, Any]]:
        ...
        pseudo_preset = {"preferred_statement_type": (family_payload.get("preferred_statement_types") or [None])[0], "core_candidates": tokens, "historical_variants": []}
        # generic_discovery currently takes a preset name. Provide a local
        # generic invocation, then enrich member identities by registry match.
        import generic_discovery as gd
        old = gd.PRESETS.get("__v67_runtime__")
        gd.PRESETS["__v67_runtime__"] = pseudo_preset
        try:
            rows = statement_discover(pdf_path, self.cache_root, display_name=family["display_name"], company=company, report_year=report_year, filing_type=filing_type, preset_name="__v67_runtime__")
        finally:
            if old is None: gd.PRESETS.pop("__v67_runtime__", None)
            else: gd.PRESETS["__v67_runtime__"] = old
        for row in rows:
