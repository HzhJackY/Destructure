...
def locate_note(index: Iterable[TextIndexRecord], *, note_reference: str, item: str, section_context: str = "") -> tuple[int | None, str, float]:
    ...
    candidates = []
    compact_reference = re.sub(r"\s+", "", str(note_reference or ""))
    # `附注11` and `附注八-11` both point to a detail heading beginning with
    # ordinal 11. The source grammar remains in the evidence record; the
    # locator compares the heading's ordinal rather than its display prefix.
    ordinal_tokens = re.findall(r"\d{1,3}", compact_reference)
    ordinal = ordinal_tokens[-1] if ordinal_tokens else compact_reference
    if not ordinal:
        return None, "NO_NOTE_ORDINAL", 0.0
    number_pattern = re.compile(rf"(?:^|\n)\s*(?:{re.escape(section_context)}[、.．-]\s*)?{re.escape(ordinal)}[、.．.\s]+[^\n]*{re.escape(item)}")
    for rec in index:
        if number_pattern.search(rec.text):
            return rec.page_number, "EXACT_HEADING", .98
        if item in rec.text and ordinal in rec.note_numbers:
