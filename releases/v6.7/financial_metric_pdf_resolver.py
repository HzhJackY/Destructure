...
class Candidate:
    ...
    evidence_detail: dict[str, float] = dataclasses.field(default_factory=dict)
...
def _source_quality(source_method: str) -> float:
    s = str(source_method or "")
    if s.startswith("pdfplumber_table"):
        base = 1.0
    elif "pymupdf_ocr" in s:
        base = 0.80
    elif s.startswith("coordinate_rows"):
        base = 0.70
    else:
        base = 0.60

    if "adjacent_numeric_recovery" in s:
        base -= 0.08
    if "cross_block" in s:
        base -= 0.05
    if "nearby_row_recovery" in s:
        base -= 0.06
    if "cross_page_header" in s:
        base -= 0.03
    return max(0.0, min(1.0, base))


def candidate_evidence_quality(
    candidate: Candidate,
    cfg: dict[str, Any],
) -> tuple[float, dict[str, float]]:
    """
    Evidence completeness is intentionally separate from semantic match score.

    A complete table row with:
      label + numeric value + period/header + unit
    must outrank an otherwise identical row that is missing unit/period.
    """
    d: dict[str, float] = {}
    ...
    # Numeric evidence.
    if values:
        parsed_ratio = sum(v.parsed_number is not None for v in values) / len(values)
        d["numeric_presence"] = 0.20
        d["numeric_parse_completeness"] = 0.08 * parsed_ratio
    else:
        d["numeric_presence"] = 0.0
        d["numeric_parse_completeness"] = 0.0

    # Period/header evidence.
    if values:
        period_ratio = sum(bool(str(v.header_context or "").strip()) for v in values) / len(values)
        strong_period_ratio = sum(v.period_score >= 0.7 for v in values) / len(values)
    else:
        period_ratio = strong_period_ratio = 0.0
    d["period_header_presence"] = 0.10 * period_ratio
    d["period_header_strength"] = 0.10 * strong_period_ratio

    # Unit evidence: particularly important for financial values.
    expected_type = str(cfg.get("value_type") or "").lower()
    if values:
        units = [str(v.unit_original or "").strip() for v in values]
        if expected_type == "percentage":
            compatible = sum(u == "%" for u in units) / len(units)
        elif expected_type == "monetary":
            compatible = sum(u in _UNIT_MULTIPLIERS for u in units) / len(units)
        else:
            compatible = sum(bool(u) for u in units) / len(units)
        unit_present = sum(bool(u) for u in units) / len(units)
    else:
        compatible = unit_present = 0.0
    d["unit_presence"] = 0.10 * unit_present
    d["unit_compatibility"] = 0.15 * compatible

    # Structural provenance.
    d["source_quality"] = 0.12 * _source_quality(candidate.source_method)
    if candidate.table_type != "未知表":
        d["known_table_type"] = 0.05
    else:
        d["known_table_type"] = 0.0

    # Explicit header-source provenance is better than an untraceable period.
    if candidate.header_source_page is not None:
        d["header_provenance"] = 0.02
    elif period_ratio > 0:
        d["header_provenance"] = 0.04
    else:
        d["header_provenance"] = 0.0

    quality = sum(d.values())
...
def enrich_candidate_quality(
    ...
    quality, detail = candidate_evidence_quality(candidate, cfg)
    # Semantic identity still dominates, but evidence quality breaks saturated ties.
    arbitration = 0.72 * float(candidate.score) + 0.28 * quality
    return dataclasses.replace(
        candidate,
        evidence_quality=quality,
        arbitration_score=round(arbitration, 6),
...
def _candidate_rank_key(candidate: Candidate) -> tuple:
    return (
        _match_class(candidate.score_detail),
        candidate.evidence_quality,
        candidate.arbitration_score,
        _source_quality(candidate.source_method),
        1 if candidate.values else 0,
        candidate.score,
...
def recover_candidate_list(
    ...
    recovered.sort(key=_candidate_rank_key, reverse=True)
    return recovered
...
def build_candidates(
    ...
    for block in blocks:
        for r, row in enumerate(block.rows):
            ...
            out.append(enrich_candidate_quality(candidate, cfg))
    out.sort(key=_candidate_rank_key, reverse=True)
    return out[:top_k]
...
def deterministic_pick(
    ...
    if exact_numeric:
        ranked_exact = sorted(exact_numeric, key=_candidate_rank_key, reverse=True)
        top = ranked_exact[0]
        second_q = ranked_exact[1].evidence_quality if len(ranked_exact) > 1 else None
        reason = (
            f"exact_label_best_evidence_quality={top.evidence_quality:.3f}"
            + (
                f"_vs_{second_q:.3f}"
        ...
        return top, top.arbitration_score, reason
    ...
    numeric_candidates.sort(key=_candidate_rank_key, reverse=True)
    top = numeric_candidates[0]
    second = (
        numeric_candidates[1].arbitration_score
        if len(numeric_candidates) > 1
        else 0.0
    )
    margin = top.arbitration_score - second
    ...
    if "qualified_variant_penalty" in top.score_detail:
        return None, top.arbitration_score, "qualified_variant_requires_review"

    if top.arbitration_score >= high_threshold:
        return top, top.arbitration_score, f"high_arbitration_score_quality={top.evidence_quality:.3f}"
    if top.arbitration_score >= medium_threshold and margin >= margin_threshold:
        return top, top.arbitration_score, f"medium_arbitration_margin={margin:.3f}_quality={top.evidence_quality:.3f}"
    return None, top.arbitration_score, f"ambiguous_arbitration={top.arbitration_score:.3f}_margin={margin:.3f}"
...
def bounded_candidate_payload(candidates: list[Candidate]) -> list[dict[str, Any]]:
    ...
    for c in candidates:
        payload.append({
            ...
            "arbitration_score": c.arbitration_score,
    ...
    for r in results:
        ...
        for i, c in enumerate(r.top_candidates[:5], 1):
            candidate_rows += (
                ...
                f"<td>{c.score:.3f}</td>"
                f"<td>{c.evidence_quality:.3f}</td>"
                f"<td>{c.arbitration_score:.3f}</td></tr>"
            )
        ...
        if s:
            ...
            selected_html = f"""
                <div class="meta-grid">
                  ...
                  <div><span>语义分</span><strong>{s.score:.3f}</strong></div>
                  <div><span>证据质量</span><strong>{s.evidence_quality:.3f}</strong></div>
                  <div><span>综合裁决分</span><strong>{s.arbitration_score:.3f}</strong></div>
                </div>
                {primary_html}
                <h4>全部数值列</h4>
        ...
        <section class="metric-card">
          ...
          {"<h4>Top 候选</h4><div class='table-wrap'><table><thead><tr><th>#</th><th>页码</th><th>原始科目</th><th>表类型</th><th>来源</th><th>语义分</th><th>证据质量</th><th>综合裁决分</th></tr></thead><tbody>"+candidate_rows+"</tbody></table></div>" if candidate_rows else ""}
        </section>
        """)
