...
当前版本主题为“报表口径优先的分级附注子表发现、验证与认证系统”。

生产主链路：

`Scope → Certified Anchor → AnchorChildConcept → FinancialNoteIndex → Tiered Retrieval → Thin Candidate → Top-K Enrichment → Reconciliation → Global Assignment → Human Review → CertifiedChildTableLink → Capture Orchestrator`

关键约束：

- 默认使用合并口径；选择 BOTH 时合并与母公司分别运行，禁止跨口径勾稽。
- 显式附注引用优先；只有不足或冲突时才进入标题语义及原始标签回退。
- 不以全 PDF 表结构或金额搜索作为召回路径。
- 未认证候选不能启动完整抓取。
- `IMPLICIT_TOTAL` 是派生合计行，不参与“末列是否具有原始 token”的适用性分母。
