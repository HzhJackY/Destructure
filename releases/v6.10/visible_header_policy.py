...
OBSERVATION_DIMENSIONS = ["company", "report_year", "data_year", "statement_scope", "restated_flag", "period_type", "currency", "currency_unit", "measure"]

DISPLAY_ORDER = ["company", "report_year", "data_year", "statement_scope", "restated_flag", "period_type", "currency", "currency_unit", "measure"]

DISPLAY_NAMES = {"company":"公司", "report_year":"报告年", "data_year":"数据年", "statement_scope":"口径", "restated_flag":"重述", "period_type":"期间", "currency":"币种", "currency_unit":"单位", "measure":"度量"}
...
class VisibleHeaderDimensionPolicy:
    ...
    def from_column_dimensions(cls, dimensions: pd.DataFrame) -> "VisibleHeaderDimensionPolicy":
        ...
        for dim in DISPLAY_ORDER:
            ...
            # report/data years remain the core time axes even for one column.
            if dim in {"report_year","data_year"}:
            ...
            if dim == "restated_flag":
                compare = [
                    "company", "report_year", "data_year", "statement_scope",
    ...
    def label_for_column(self, row: dict[str, Any]) -> dict[str, str]:
        ...
        if "restated_flag" not in self.visible_header_dimensions and _flag(row.get("restated_flag")) and "data_year" in labels:
            labels["data_year"]=(labels["data_year"] + "（已重述）").strip()
        return labels
...
def adaptive_wide_preview_html(
    ...
    *,
    ...
    header_dimensions = list(policy.visible_header_dimensions) or ["data_year"]
