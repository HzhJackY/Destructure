...
def locate_primary_statements(index: Iterable[TextIndexRecord]) -> dict[str, list[int]]:
    """Locate formal statement pages without treating narrative mentions as titles.
    ...
    fallback when an unusual PDF exposes no formal title at all.
    """
    found: dict[str, list[int]] = {key: [] for key in STATEMENT_PATTERNS}
    broad: dict[str, list[int]] = {key: [] for key in STATEMENT_PATTERNS}
    for rec in index:
        title_lines = [re.sub(r"\s+", "", line) for line in rec.text.splitlines()[:32] if line.strip()]
        for kind, patterns in STATEMENT_PATTERNS.items():
            if any(token in rec.text[:2500] for token in patterns):
                broad[kind].append(rec.page_number)
            if any(
                (clean := line.replace("（续）", "").replace("(续)", "").replace("续表", "").strip("：:")) in patterns
...
def locate_note(index: Iterable[TextIndexRecord], *, note_reference: str, item: str, section_context: str = "") -> tuple[int | None, str, float]:
    ...
    detail context.  We intentionally return a review status (rather than a
    guessed page) for weaker retrieval evidence.
    """
    candidates = []
    ...
    for rec in index:
        if not _is_contents_record(rec) and item in rec.text and ordinal in rec.note_numbers:
