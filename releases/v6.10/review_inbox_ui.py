...
def render_review_inbox(st, backend) -> None:
    ...
    if anchor_rows:
        with st.expander(f"主报表 Anchor 选择（{len(anchor_rows)}）",expanded=True):
            anchor_display=[{
    ...
    if child_rows:
        with st.expander(f"子表映射选择（{len(child_rows)}）",expanded=True):
            child_display=[{
                "主表子项":row["raw_label"],"口径":row["statement_scope"],
                "候选数":len(row["candidate_ids"]),
                "原因":"没有唯一高置信附注子表，需要人工映射",
                "来源 PDF":row["source_pdf_id"],
