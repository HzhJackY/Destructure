...
# -----------------------------------------------------------------------------
    ...
    if monitor_batch:
        ...
        if summary["total"] and summary["complete"]+summary["counts"].get("FAILED",0)==summary["total"]:
            batch_captures=BACKEND.asset_service.list_captures(
                batch_id=monitor_batch,include_trash=False,limit=2000,
            )
            review_queue=[]
            for capture in batch_captures:
                detail=BACKEND.capture_version_service.detail(str(capture["capture_id"])) or {}
                if (
                    detail.get("logical_asset_id")
                    and (
                        str(detail.get("quality_status") or "")=="REVIEW_REQUIRED"
                        or str(detail.get("review_status") or "")=="REVIEW_REQUIRED"
                    )
                ):
                    review_queue.append({
                        "capture_id":str(capture["capture_id"]),
                        "logical_asset_id":str(detail["logical_asset_id"]),
                        "company_id":detail.get("company_id") or capture.get("company"),
                        "report_year":detail.get("report_year") or capture.get("document_year"),
                        "table_family_id":detail.get("table_family_id"),
                        "member_table_id":detail.get("member_table_id") or capture.get("table_query"),
                        "initial_tab":"审核","return_route":"整表批量工作台",
                    })
            if review_queue:
                st.warning(
                ...
                if st.button(
                    ...
                    st.rerun()
            elif not summary["counts"].get("FAILED",0):
                st.success("本批次作业已完成，当前没有待审核 Capture。")
...
被截断的后续数据保存在 boundary_excluded_rows.csv。""",
    ...
    if blocked_records:
        with st.expander(f"有 {len(blocked_records)} 个整表抓取尚未通过结构认证，暂不可合表", expanded=False):
            ...
            blocked_option_map = {
                f"{r.get('display_name')} · {r.get('run_id')}": r
                for r in blocked_records
            }
