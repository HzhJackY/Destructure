...
from statement_note_navigation import TextIndexRecord, build_text_index, locate_primary_statements, locate_note
...
def _is_target_line(line: str, display_name: str) -> bool:
    ...
    terminal = stripped[len(target):].strip("：:;-—－") if stripped.startswith(target) else ""
    return stripped.startswith(target) and (
        not terminal
        or bool(re.search(r"[-(（]?\d[\d,，.]*", stripped[len(target):]))
    )


def _infer_statement_scope(text: str, statement_type: str) -> str:
    compact = normalize_text("\n".join(str(text or "").splitlines()[:18]))
    titles = {
        "BALANCE_SHEET": "资产负债表",
        "INCOME_STATEMENT": "利润表",
        "CASH_FLOW": "现金流量表",
    }
    title = titles.get(statement_type, "")
    # Listed insurers often print one combined page as “合并及公司资产负债表”.
    # It still contains the consolidated lane first; do not misclassify the
    # page as parent-company merely because the two scope labels share a title.
    if title and (f"合并{title}" in compact or f"合并及公司{title}" in compact):
        return "CONSOLIDATED"
...
def _qualified_target_pages(index, statements: dict[str, list[int]], statement_types: list[str],
    ...
    def target_quality(rec) -> tuple[str, dict[str, Any]]:
        ...
        for line_index, line in enumerate(lines):
            ...
            if "附注" not in compact_page or not note_header:
                continue
            tail = line.split(display_name, 1)[-1]
            if _standalone_note_ordinal(tail):
        ...
        if require_note_reference and note_header and core:
            ...
            for line_index, line in _logical_statement_lines(lines):
                ...
                for candidate in core:
                    if candidate and compact.startswith(candidate):
    ...
    for statement_type in candidate_types:
        ...
        for rec in index:
            ...
            for line_index, raw_line in enumerate(page_lines):
                ...
                statement_amounts = _nearby_statement_amount_observations(page_lines, line_index, matched)
                ...
                if note_ref:
                    ...
                    if note.get("note_reference_item"):
                        inferred_section = _infer_note_section_from_target_page(
                            index, page, _infer_statement_scope(rec.text, statement_type)
                        )
                        if inferred_section:
                ...
                rows.append({
                    ...
                    "scope": _infer_statement_scope(rec.text, statement_type),
                    ...
                    "ocr_engine": ocr_audit.get("ocr_engine") if rec.page_number in page_modes else "",
                    "ocr_confidence": ocr_audit.get("max_ocr_candidate_score") if rec.page_number in page_modes else None,
