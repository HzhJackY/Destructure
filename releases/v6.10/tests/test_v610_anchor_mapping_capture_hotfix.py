...
def test_sole_viable_child_mapping_is_preselected_for_review_form():
    ...
    rows = service.link_candidates(anchor, child, enriched, {"member_table_id": "debt_investment"})
    assert len(rows) == 1
    assert rows[0]["is_preselected"] is True
    assert rows[0]["preselection_reason"] == "SOLE_VIABLE_CANDIDATE"
...
def test_missing_note_ordinal_does_not_invent_a_peer_hard_boundary():
    ...
    assert boundary["boundary_reason"] == "boundary_unresolved"
