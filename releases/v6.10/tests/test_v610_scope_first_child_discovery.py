...
def test_schema_migration_is_idempotent_and_review_queue_is_durable():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        queue = repo.child_review_queue()
        assert len(queue) == 1
        assert queue[0]["raw_label"] == "债权投资"
        assert queue[0]["candidate_ids"] == ["C1", "C2"]
...
def test_global_assignment_never_auto_certifies_ambiguous_candidate():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert assignment["decisions"][0]["status"] == "CHILD_TABLE_SELECTION_REQUIRED"
        assert repo.child_review_queue()[0]["status"] == "PENDING"
