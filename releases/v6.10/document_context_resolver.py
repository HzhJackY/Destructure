...
_UNIT_PATTERNS = [
    re.compile(r"金额单位\s*(?:为|：|:)\s*(?:人民币\s*)?(百万元|亿元|万元|千元|元)"),
    re.compile(r"(?:除特别注明外[，,]?)?金额(?:单位)?\s*(?:为|以)\s*(?:人民币\s*)?(百万元|亿元|万元|千元|元)(?:列示)?"),
]
    ...
    def resolve(self, page_no: int) -> DocumentContext:
        ...
        for index in range(page_no):
            ...
            if unit:
                ...
                context.declarations.append({"kind": "unit", "value": unit, "source_page": source_page})
