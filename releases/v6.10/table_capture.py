...
def capture_to_long_df(result: TableCaptureResult) -> pd.DataFrame:
    ...
    section_ids = {row.normalized_item: row_ids[row.row_order] for row in result.rows if row.normalized_item and row.row_type == "SECTION"}
        ...
        if not row.cells:
            records.append({
                **common,
                ...
                "parent_row_id": section_ids.get(row.parent_section),
...
def write_capture_artifacts(
    ...
    result.boundary_status = derive_boundary_status(result.to_dict())
