...
class ResearchBatchService:
    ...
    def create(self, display_name:str, table_family:str, payload:dict[str,Any]|None=None, *, research_definition_id:str|None=None, definition_version:str|None=None)->dict[str,Any]:
        # Pin the selected version in immutable batch payload. Later registry
        # edits create a new version and cannot silently change old research.
        effective_payload=dict(payload or {})
        if research_definition_id: effective_payload['research_definition_id']=research_definition_id
        if definition_version: effective_payload['definition_version']=definition_version
        item={'research_batch_id':'RB_'+uuid.uuid4().hex,'display_name':display_name,'table_family':table_family,'status':'ACTIVE','payload':effective_payload,'created_at':now()}
    ...
    def backfill_missing_definition_identity(self, research_batch_id: str) -> dict[str, Any]:
        """Repair only missing identity fields from this batch's pinned payload.

        This is deliberately a NULL-only backfill: it never replaces an
        existing capture/asset definition with the current registry version.
        """
        with self.registry.connect() as c:
            batch = c.execute(
            ...
            payload = json.loads(dict(batch).get('payload_json') or '{}') if batch else {}
            definition_id = str(payload.get('research_definition_id') or '')
            definition_version = str(payload.get('definition_version') or '')
            if not definition_id or not definition_version:
                return {'updated': 0, 'reason': 'BATCH_DEFINITION_NOT_PINNED'}
            rows = c.execute(
                '''SELECT DISTINCT cv.logical_asset_id FROM capture_versions cv
                   JOIN captures cap ON cap.capture_id=cv.capture_id
                   JOIN research_batch_members rb ON rb.source_batch_id=cap.batch_id
                   WHERE rb.research_batch_id=?''',
                (research_batch_id,),
            ).fetchall()
            ids = [str(row['logical_asset_id']) for row in rows if row['logical_asset_id']]
            updated = 0
            for asset_id in ids:
                cur = c.execute(
                    '''UPDATE logical_assets
                       SET research_definition_id=COALESCE(NULLIF(research_definition_id,''),?),
                           definition_version=COALESCE(NULLIF(definition_version,''),?)
                       WHERE logical_asset_id=?
                         AND (COALESCE(research_definition_id,'')='' OR COALESCE(definition_version,'')='')''',
                    (definition_id, definition_version, asset_id),
                )
                updated += int(cur.rowcount or 0)
        return {'updated': updated, 'definition_id': definition_id, 'definition_version': definition_version}
