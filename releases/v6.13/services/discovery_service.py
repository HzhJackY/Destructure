    ...
    def rank_anchor_candidates(
        ...
        for raw in occurrences:
            ...
            direct_portfolio = bool(
                row.get("direct_portfolio_table")
                or evidence.get("strategy") == "DIRECT_PORTFOLIO_TABLES"
            )
            # A directly disclosed portfolio table is not a financial main
            # statement.  Its resolver already supplies bounded page/ROI,
            # period, unit and amount-lane evidence.  Running the main-
            # statement geometry parser here can relabel it as a balance sheet
            # and reintroduce the old Anchor/note route.
            if pdf.is_file() and page and not direct_portfolio:
                try:
