...
def _row_values(row:dict[str,Any])->list[Any]:
    ...
    if isinstance(cells,list):
        ordered=sorted(
            cells,
            key=lambda cell:int(cell.get("column_ordinal"))
            if isinstance(cell,dict) and cell.get("column_ordinal") is not None
            else 0,
        )
        ordinals=[
            int(cell.get("column_ordinal"))
            for cell in ordered
            if isinstance(cell,dict)
            and cell.get("column_ordinal") is not None
            and int(cell.get("column_ordinal")) >= 0
        ]
        if ordinals and len(ordinals) == len(ordered) and len(set(ordinals)) == len(ordinals):
            values=[None] * (max(ordinals) + 1)
            for ordinal, cell in zip(ordinals, ordered):
                values[ordinal]=(
                    cell.get("raw")
...
def _row_has_value_at_column(row:dict[str,Any], column_index:int)->bool:
    cells=row.get("cells")
    if isinstance(cells,list):
        for cell in cells:
            if not isinstance(cell,dict):
                continue
            try:
                ordinal=int(cell.get("column_ordinal"))
            except (TypeError,ValueError):
                continue
            if ordinal != column_index:
                continue
            raw=cell.get("raw")
            if raw is not None and str(raw).strip():
                return True
            if cell.get("parsed_number") is not None:
                return True
        return False
    values=_row_values(row)
    return column_index < len(values) and bool(str(values[column_index] or "").strip())
...
def review_final_data_columns(result:dict[str,Any])->dict[str,Any]:
    ...
    applicable_rows=[
        row for row in rows
        if not row.get("excluded_from_table_logic")
    ...
    if last:
        ...
        last_tokens=sum(
            _row_has_value_at_column(row,last_position)
