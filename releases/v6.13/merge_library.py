...
def research_wide_download_name(display_name: Any, extension: str = "xlsx") -> str:
    """Build a user-facing filename without changing stable on-disk artifacts."""
    safe_name = re.sub(r'[\\/:*?"<>|]+', "_", str(display_name or "").strip())
    safe_name = safe_name.strip(" ._")[:180].rstrip(" ._") or "合表项目"
    safe_extension = str(extension or "xlsx").strip().lstrip(".") or "xlsx"
    return f"{safe_name}_研究宽表.{safe_extension}"
