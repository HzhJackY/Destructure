...
def test_report_footer_without_duplicate_year_character_is_filtered_before_amount_parsing() -> None:
    footer = _line(754.627, [
        _word(485.024, 499.220, "2025", 754.627),
        _word(500.600, 524.240, "年度报告", 754.627),
        _word(546.803, 555.011, "59", 754.627),
    ])

    assert _report_page_chrome_role(footer, 841.89) == "PAGE_FOOTER_NOISE"


def test_explicit_whereof_parent_allows_same_indent_but_generic_section_does_not() -> None:
    assert _section_parent_allows_same_indent("其中：") is True
    assert _section_parent_allows_same_indent("债权型投资") is False


def test_certified_legacy_financial_sections_form_nested_parent_chain() -> None:
    def section(order: int, label: str) -> TableRow:
        return TableRow(
            row_order=order, page=176, block_id="BLOCK_1",
            source_method="TEST", raw_item=label, normalized_item=label,
            canonical_item=None, mapping_status="UNMAPPED",
            row_type="PARENT_SECTION", row_role="PARENT_SECTION",
            row_level=0, parent_section=None, cells=[],
            header_source_page=None,
            source_row_id=f"ROW_{order}",
        )

    def detail(order: int, label: str, parent: str) -> TableRow:
        return TableRow(
            row_order=order, page=176, block_id="BLOCK_1",
            source_method="TEST", raw_item=label, normalized_item=label,
            canonical_item=None, mapping_status="UNMAPPED",
            row_type="DETAIL", row_role="DETAIL", row_level=1,
            parent_section=parent, cells=[_cell(0, 1.0)],
            header_source_page=None,
            source_row_id=f"ROW_{order}",
        )

    fair_value = section(1, "以公允价值计量的可供出售金融资产")
    debt = section(2, "债权型投资")
    bond = detail(3, "国债", "债权型投资")
    equity = section(4, "股权型投资")
    stock = detail(5, "股票", "股权型投资")
    subtotal = detail(6, "小计", "股权型投资")
    subtotal.row_type = "SUBTOTAL"
    subtotal.row_role = "SUBTOTAL"
    cost = section(7, "以成本计量的可供出售金融资产")
    cost_equity = section(8, "股权型投资")
    other = detail(9, "其他", "股权型投资")

    _finalize_hierarchy_semantics([
        fair_value, debt, bond, equity, stock, subtotal, cost, cost_equity, other,
    ])

    assert debt.parent_row_id == fair_value.source_row_id
    assert debt.row_level == 1
    assert bond.parent_row_id == debt.source_row_id
    assert bond.row_level == 2
    assert equity.parent_row_id == fair_value.source_row_id
    assert stock.parent_row_id == equity.source_row_id
    assert subtotal.parent_row_id == equity.source_row_id
    assert cost.parent_row_id is None
    assert cost_equity.parent_row_id == cost.source_row_id
    assert other.parent_row_id == cost_equity.source_row_id
    assert other.row_level == 2
