...
import generic_discovery_engine as generic_discovery_engine_module
...
600"""
    ...
    assert resolution["family_total_status"] == "NOT_REPORTED"
    # Under the legacy presentation, term deposits are a registered direct
    # financial-investment member.  Long-term equity remains outside the
    # family.  A newer explicit-parent presentation is covered separately and
    # must still not absorb deposits from outside that physical parent block.
    assert len(rows) == 5
    ...
    assert set(resolution["outside_family_members"]) == {"long_term_equity"}
    assert "time_deposits" in set(resolution["member_ids"])
    assert "long_term_equity" not in set(resolution["member_ids"])


def test_cpic_transition_duplicate_member_keeps_row_level_period_identity():
    text = """合并资产负债表
资产 附注七 2023年12月31日 2022年12月31日 2022年1月1日
以公允价值计量且其变动计入当期损益的金融资产 2 - 26,560 12,353
金融投资： 2,009,336 1,596,156 1,470,000
交易性金融资产 10 581,602 - -
债权投资 11 82,334 - -
其他债权投资 12 1,247,435 - -
其他权益工具投资 13 97,965 - -
长期股权投资 14 1,000 900 800"""
    rows, resolutions = StatementFamilyResolver().resolve(
        index=_index(text), family=_family(), members=_members(),
        company="中国太保", report_year="2023", filing_type="ANNUAL_REPORT",
    )

    fvtpl_rows = [row for row in rows if row["member_table"] == "fvtpl_assets"]
    assert len(fvtpl_rows) == 2
    status_by_label = {
        row["raw_member_label"]: row["member_period_status"] for row in fvtpl_rows
    }
    assert status_by_label["以公允价值计量且其变动计入当期损益的金融资产"] == (
        "COMPARATIVE_ONLY_LEGACY_MEMBER"
    )
    assert status_by_label["交易性金融资产"] == "ACTIVE_CURRENT_PERIOD"
    assert resolutions[0]["member_period_statuses"]["fvtpl_assets"] == (
        "ACTIVE_CURRENT_PERIOD"
    )
    assert set(resolutions[0]["evidence"]["row_period_statuses"].values()) >= {
        "COMPARATIVE_ONLY_LEGACY_MEMBER", "ACTIVE_CURRENT_PERIOD",
    }
...
def test_ocr_unreadable_parent_never_certifies_implicit_member_set():
    ...
    assert resolution["review_status"] == "REVIEW_REQUIRED_ACTIONABLE"
...
def test_generic_discovery_preserves_resolver_member_identity_through_child_link(
    monkeypatch,
    tmp_path,
):
    """A resolved legacy identity must not be migrated by display-label fallback."""
    family = {
        **_family(),
        "discovery_strategy": "STATEMENT_ITEM_TO_NOTE_FAMILY",
    }
    members = _members()
    resolution = {
        "resolution_id": "SFR_LEGACY_IDENTITY",
        "quality_status": "RESOLVED",
        "resolution_mode": "IMPLICIT_MEMBER_SET",
        "presentation_regime": "LEGACY_FINANCIAL_ASSET_CLASSIFICATION",
        "required_current_members": ["legacy_fvtpl_assets"],
        "member_ids": ["legacy_fvtpl_assets"],
    }
    resolved_row = {
        "discovery_id": "MD_LEGACY_IDENTITY",
        "pdf_id": "PDF_LEGACY_IDENTITY",
        "statement_type": "BALANCE_SHEET",
        "scope": "CONSOLIDATED",
        "statement_pdf_page_index": 1,
        "source_table_title": "合并资产负债表",
        "statement_item": "以公允价值计量且其变动计入当期损益的金融资产",
        "member_table": "legacy_fvtpl_assets",
        "canonical_concept_id": "legacy_fvtpl_assets",
        "statement_family_member_id": "SFM_LEGACY_IDENTITY",
        "statement_family_resolution_id": resolution["resolution_id"],
        "presentation_regime": resolution["presentation_regime"],
    ...
    class _Definitions:
        def definition(self, definition_id):
            return {
                ...
                "status": "ACTIVE",
                "payload": {"table_families": ["financial_investment"]},
            }
        ...
        def members(self, family_id):
            assert family_id == "financial_investment"
            return members

    class _Resolver:
        def resolve(self, **_kwargs):
            return [dict(resolved_row)], [dict(resolution)]

    monkeypatch.setattr(
        generic_discovery_engine_module,
        "build_text_index",
        lambda *_args, **_kwargs: [],
    )
    service = GenericDiscoveryService(_Definitions(), tmp_path)
    ...
    selected = result["family_resolutions"][0]
    assert selected["required_current_members"] == ["legacy_fvtpl_assets"]
    assert selected["member_ids"] == ["legacy_fvtpl_assets"]
    assert [row["member_table"] for row in result["candidates"]] == [
        "legacy_fvtpl_assets"
    ]
    assert [row["canonical_concept_id"] for row in result["candidates"]] == [
        "legacy_fvtpl_assets"
    ]
    assert [
        child["member_table"]
