...
def test_lines_in_roi_excludes_same_page_sibling_by_certified_bbox() -> None:
    ...
    lines = [
        ...
        # Font bbox crosses the certified tail, but its row anchor remains in.
        {"x0": 30.0, "x1": 480.0, "y0": 230.0, "y1": 246.0},
        # Geometrically overlaps the tail, but its row anchor is outside.
        {"x0": 30.0, "x1": 480.0, "y0": 238.0, "y1": 250.0},
    ...
    assert [line["y0"] for line in selected] == [120.0, 230.0]
...
def test_certified_bbox_identity_allows_missing_title(tmp_path: Path) -> None:
    ...
    assert result["certified_roi_row_membership_semantics"] == (
