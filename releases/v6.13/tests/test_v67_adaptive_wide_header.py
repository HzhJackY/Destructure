...
def main():
    ...
    # third header level.
    assert labels["data_year"] == "2022" and labels.get("restated_flag") == "已重述"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert ws.cell(row=3, column=6).value == "2022"
        assert ws.cell(row=4, column=6).value == "已重述"
        assert ws.cell(row=6, column=5).value == 380239
