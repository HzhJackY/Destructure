...
def test_portfolio_members_have_ratio_reconciliation() -> None:
    ...
    direct_members = [
        ...
        if member.get("member_role") == "DIRECT_DISCLOSURE_TABLE"
        and member.get("activation") != "NUMERIC_PREFIX_BEFORE_FIRST_CERTIFIED_AXIS"
    ]
    for m in direct_members:
        assert m.get("ratio_total_reconciliation") is True, f"{m['member_id']} missing ratio reconciliation"
    components = next(
        member for member in members
        if member["member_id"] == "portfolio_components"
    )
    assert components["ratio_total_reconciliation"] is False
    assert components["total_policy"] == "NO_SYNTHETIC_PORTFOLIO_TOTAL"
...
def test_topology_contract_has_no_universal_required_physical_member() -> None:
    contract = INVESTMENT_PORTFOLIO_TOPOLOGY_CONTRACT
    assert contract["runtime_activation_status"] == "ACTIVE_FOR_INVESTMENT_PORTFOLIO_V2"
    assert contract["resolver_implementation_status"] == "IMPLEMENTED_NODE_4_NATIVE_FIRST"
    assert contract["universal_required_members"] == []


def test_single_axis_topology_marks_measurement_not_applicable() -> None:
    topology = INVESTMENT_PORTFOLIO_TOPOLOGY_CONTRACT["topologies"][
        "DIRECT_SINGLE_AXIS_TABLE"
    ]
    assert topology["required_members"] == ["portfolio_by_category"]
    assert topology["not_applicable_members"] == ["portfolio_by_measurement"]


def test_multi_note_fallback_forbids_synthetic_portfolio_total() -> None:
    topology = INVESTMENT_PORTFOLIO_TOPOLOGY_CONTRACT["topologies"][
        "MULTI_NOTE_COMPONENT_SET_NO_REPORTED_TOTAL"
    ]
    assert topology["reported_total_policy"] == "NOT_DISCLOSED_NO_SYNTHETIC_TOTAL"
    assert topology["statement_link_policy"] == "CERTIFIED_CHILD_TABLE_LINKS_ONLY"
