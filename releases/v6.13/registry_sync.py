    ...
    def sync_pdfs(self) -> dict[str, Any]:
        ...
        with self.registry.connect() as conn:
            if seen_ids:
                ...
                conn.execute(
                    f"DELETE FROM pdf_assets WHERE COALESCE(lifecycle_status,'ACTIVE')='ACTIVE' "
                    f"AND pdf_id NOT IN ({marks})",
                    tuple(sorted(seen_ids)),
                )
            else:
                conn.execute(
                    "DELETE FROM pdf_assets WHERE COALESCE(lifecycle_status,'ACTIVE')='ACTIVE'"
                )
