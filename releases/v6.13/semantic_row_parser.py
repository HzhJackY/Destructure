...
NOTE_ITEM_RE = re.compile(r"^\s*(?:注\s*[：:]|[（(]\s*\d{1,3}\s*[）)]|\d{1,3}\s*[.．、])")
MEMO_TOKENS = (
    "以下名称", "以下简称", "截至", "包括", "上述", "不构成",
    # Wrapped disclosure prose often starts on the preceding line and leaves
    # only a cross-reference fragment on the final line.
    "请见附注",
)
...
def classify_non_data_text(
    ...
    *,
    ...
    if (
        (raw.startswith(NOTE_PREFIXES) or (len(raw) >= 6 and bool(NOTE_ITEM_RE.match(str(text or "").strip()))))
        and insufficient
    ):
        return "NOTE_TEXT"
