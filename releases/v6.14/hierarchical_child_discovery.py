...
ENRICHMENT_VERSION="LOCAL_TOPK_V5_CERTIFIED_OCR_GEOMETRY_REPLAY"
...
def _freeze_certified_ocr_geometry(
    structure:dict[str,Any], *, page_number:int,
    ocr_lines:list[dict[str,Any]], recovery_audit:dict[str,Any],
)->dict[str,Any]:
    """Attach replayable point geometry only to certified segment candidates.

    The inventory continues to carry no parsed amount values. This is the
    bounded word/BBox evidence used to produce the already-validated column
    signatures, so formal Capture can replay the same physical geometry rather
    than silently falling back to incomplete Native words.
    """
    words=[
        [
            float(word["x0"]),float(word["y0"]),
            float(word["x1"]),float(word["y1"]),str(word["text"]),
        ]
    ...
    if not words:
        raise ValueError("CERTIFIED_OCR_GEOMETRY_WORDS_REQUIRED")
    prepared=dict(structure)
    prepared_segments=[]
    attached=0
    for raw in structure.get("segment_candidates") or []:
        segment=dict(raw)
        if int(segment.get("start_page") or 0)==int(page_number):
            evidence=dict(segment.get("evidence") or {})
            evidence.update({
                "source":"NATIVE_TITLE_FAST_INDEX_OCR_STRUCTURE_PDF_POINTS",
                "certified_column_geometry_source":(
                    "FAST_INDEX_OCR_WORDS_PDF_POINTS"
                ),
                "certified_ocr_geometry_contract_version":1,
                "certified_ocr_words_pdf_points":words,
                "ocr_recovery_stage":"CANDIDATE_EVIDENCE_RECOVERY",
                ...
                "ocr_cache_identity":str(
                    recovery_audit.get("cache_identity")
                    or recovery_audit.get("cache_key") or ""
                ),
            })
    ...
    if attached!=1:
        raise ValueError("CERTIFIED_OCR_GEOMETRY_SEGMENT_CARDINALITY_REQUIRED")
    prepared["segment_candidates"]=prepared_segments
    return prepared
...
def _financial_note_family_titles() -> list[str]:
    """Return registry-owned financial-note titles for peer-boundary checks.

    The list is intentionally derived from the Research Definition registry,
    not from a company/year/page rule.  It includes optional legacy members so
    a current IFRS 9 table can still be bounded by the following legacy or
    non-primary family table during a transition filing.
    """
    from research_definition_registry import BUILTIN_MEMBERS

    titles: list[str] = []
    for member in BUILTIN_MEMBERS.get("financial_investment") or []:
        if str(member.get("member_role") or "") == "STATEMENT_ANCHOR":
            continue
        titles.extend([
            str(member.get("display_name") or ""),
            *(str(value) for value in member.get("aliases") or []),
        ])
    return list(dict.fromkeys(value for value in titles if value.strip()))


def _exact_title_line(
    lines: list[dict[str, Any]], title: str,
) -> dict[str, Any] | None:
    target = _norm(title)
    matches = [
        line for line in lines
        if target and _norm(line.get("text") or "") == target
    ]
    if len(matches) != 1:
        return None
...
def _source_target_local_context(
    doc: fitz.Document,
    *,
    start_page: int,
    table_title: str,
) -> dict[str, Any] | None:
    """Resolve a source-target title and its next registry peer locally.

    Source-resolved candidates already carry a concrete physical page.  A
    global leaf-ordinal lookup is unsafe here because the same leaf number can
    occur in accounting-policy, tax and statement-note sections.  The local
    resolver therefore keeps the source page fixed, requires one exact Native
    title, and searches at most the current and following page for the next
    known table-family title.
    """
    from spatial_table_capture import _page_lines

    page_number = int(start_page)
    if page_number < 1 or page_number > len(doc):
        return None
    current_lines = _page_lines(doc, page_number)
    title_line = _exact_title_line(current_lines, table_title)
    if title_line is None:
        return None

    current_key = _norm(table_title)
    peer_keys = {
        _norm(value): value for value in _financial_note_family_titles()
        if _norm(value) and _norm(value) != current_key
    }
    peer_hits: list[tuple[int, dict[str, Any], str]] = []
    for candidate_page in range(
        page_number, min(len(doc), page_number + 1) + 1
    ):
        for line in _page_lines(doc, candidate_page):
            if (
                candidate_page == page_number
                and float(line.get("y0") or 0.0)
                <= float(title_line.get("y1") or 0.0)
            ):
                continue
            key = _norm(line.get("text") or "")
            if key in peer_keys:
                peer_hits.append((candidate_page, dict(line), peer_keys[key]))
    peer = min(
        peer_hits,
        key=lambda item:(item[0], float(item[1].get("y0") or 0.0)),
        default=None,
    )
    return {
        "title_bbox": {
            key: float(title_line[key]) for key in ("x0", "y0", "x1", "y1")
        },
        "title_line": str(title_line.get("text") or ""),
        "next_peer_page": int(peer[0]) if peer else None,
        "next_peer_bbox": ({
            key: float(peer[1][key]) for key in ("x0", "y0", "x1", "y1")
        } if peer else {}),
        "next_peer_title": str(peer[2]) if peer else "",
    }


def _post_total_narrative_boundary_y(
    lines: list[dict[str, Any]],
    *,
    title_bottom: float,
    peer_y0: float | None = None,
) -> float | None:
    """Find a conservative prose boundary after a witnessed total row.

    Numeric prose is allowed: dates and amounts in a paragraph must not create
    a third table lane.  The rule remains fail-closed by requiring a prior
    numeric 合计/小计 row plus a long punctuated prose line.  Tables without a
    total are left to the independently verified next-peer boundary.
    """
    witnessed_total = False
    for line in lines:
        y0 = float(line.get("y0") or 0.0)
        if y0 <= float(title_bottom):
            continue
        if peer_y0 is not None and y0 >= float(peer_y0):
            break
        compact = re.sub(r"\s+", "", str(line.get("text") or ""))
        if not compact:
            continue
        has_number = any(
            bool(re.search(r"\d", str(word.get("text") or "")))
            for word in line.get("words") or []
        )
        if compact.startswith(("合计", "小计")) and has_number:
            witnessed_total = True
            continue
        if not witnessed_total:
            continue
        ...
        if (
            len(compact) >= 20
            and any(mark in compact for mark in ("，", "。", "；", ";"))
        ):
            return y0
    return None


def _source_target_boundary_contract(
    context: dict[str, Any],
    *,
    start_page: int,
    narrative_y0: float | None = None,
) -> dict[str, Any]:
    peer_page = context.get("next_peer_page")
    peer_bbox = dict(context.get("next_peer_bbox") or {})
    peer_verified = bool(peer_page and peer_bbox)
    same_page_peer_y0 = (
        float(peer_bbox["y0"])
        if peer_verified and int(peer_page) == int(start_page)
        else None
    )
    end_candidates = [
        value for value in (narrative_y0, same_page_peer_y0)
        if value is not None
    ]
    end_y = min(end_candidates) - 0.01 if end_candidates else None
    narrative_selected = bool(
        narrative_y0 is not None
        and (same_page_peer_y0 is None or narrative_y0 < same_page_peer_y0)
    )
    complete = narrative_selected or peer_verified
    return {
        "boundary_reason": (
            "post_total_narrative_separator"
            if narrative_selected else "next_peer_heading"
            if peer_verified else "source_target_boundary_unresolved"
        ),
        "boundary_confidence": "HIGH" if complete else "LOW",
        "boundary_status": (
            "HARD_BOUNDARY_CONFIRMED"
            if narrative_selected else "VERIFIED_PEER_BOUNDARY"
            if peer_verified else "UNRESOLVED"
        ),
        "inventory_boundary_complete": complete,
        "end_y": end_y,
        "boundary_evidence": {
            "method": (
                "POST_TOTAL_NARRATIVE_SEPARATOR"
            ...
            "next_note_pdf_page_index": int(peer_page) if peer_page else None,
            "next_note_y0": (
                float(peer_bbox["y0"]) if peer_bbox.get("y0") is not None else None
            ),
            "next_note_bbox": peer_bbox,
            "next_note_title": context.get("next_peer_title") or "",
            "narrative_separator_y0": (
                float(narrative_y0) if narrative_y0 is not None else None
            ),
            "source_title_line": context.get("title_line") or "",
        },
    }


def _repair_compact_ocr_period_words(
    words: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Reconstruct compact OCR dates without inventing a filing period.

    Tesseract can collapse ``2025年12`` to ``2025712`` and ``月31日`` to
    ``A318``.  Recovery is allowed only when a word already starts with an
    explicit 20xx year, ends in a plausible month, and the immediately
    adjacent word starts with a plausible day.  The source tokens and their
    BBoxes provide every date component; the helper merely restores separators
    before the shared period normalizer runs.
    """
    ordered = sorted(
        (dict(word) for word in words),
        key=lambda word:(float(word.get("y0") or 0.0), float(word.get("x0") or 0.0)),
    )
    ...
    for index, word in enumerate(ordered):
        ...
        month = None
        for width in (2, 1):
            if len(tail_digits) < width:
                continue
            value = int(tail_digits[-width:])
            if 1 <= value <= 12:
                month = value
                break
        if month is None:
            continue
        neighbours = [
            (next_index, next_word)
            for next_index, next_word in enumerate(ordered)
            if next_index not in consumed and next_index != index
            and 0.0 <= float(next_word.get("x0") or 0.0) - float(word.get("x1") or 0.0) <= 45.0
            and abs(float(next_word.get("yc") or 0.0) - float(word.get("yc") or 0.0)) <= 8.0
        ]
        if not neighbours:
            continue
        next_index,next_word=min(
            neighbours,key=lambda item:float(item[1].get("x0") or 0.0)
        )
        next_raw=re.sub(r"\s+", "", str(next_word.get("text") or ""))
        digit_runs=re.findall(r"\d+", next_raw)
        day=None
        ...
        reconstructed.append({
            "x0":min(float(word["x0"]),float(next_word["x0"])),
            "y0":min(float(word["y0"]),float(next_word["y0"])),
            "x1":max(float(word["x1"]),float(next_word["x1"])),
            "y1":max(float(word["y1"]),float(next_word["y1"])),
            "xc":(
                min(float(word["x0"]),float(next_word["x0"]))
                +max(float(word["x1"]),float(next_word["x1"]))
            )/2.0,
            "yc":(
                min(float(word["y0"]),float(next_word["y0"]))
                +max(float(word["y1"]),float(next_word["y1"]))
            )/2.0,
            "text":f"{year}年{month:02d}月{day:02d}日",
            "block":0,"line":0,"word":min(index,next_index),
            "source_ocr_tokens":[raw,next_raw],
        })
    return [
        word for index,word in enumerate(ordered) if index not in consumed
    ]+reconstructed
    ...
    def certify_direct_portfolio_table(
        ...
        *,
        ...
        certified_topology = build_certified_column_topology_from_pdf(
            ...
            ocr_cache_root=self.registry.db_path.parent / "cache",
        )
...
class HierarchicalChildTableDiscoveryService:
    ...
    def _inventory_payload(
        ...
        return {
            ...
            "evidence":{
                "source":structure.get("evidence_source") or "NATIVE_PDF_LINES",
                "recovery_stage":structure.get("recovery_stage") or "NATIVE_DISCOVERY",
                "ocr_recovery_audit":dict(
                    structure.get("ocr_recovery_audit") or {}
                ),
    ...
    def enrich_top_k(self,pdf_path:Path,child:dict[str,Any],
        ...
        from spatial_table_capture import (
            _group_lines,_page_lines,plan_table_structure_candidates,
        )
        with fitz.open(str(pdf_path)) as doc:
            for candidate in ranked[:self.limits["ENRICHMENT_TOP_K"]]:
                ...
                source_target=bool(
                    str(candidate.get("heading_id") or "").startswith(
                        "SOURCE_NOTE_TARGET::"
                    )
                )
                local_context=(
                    _source_target_local_context(
                        doc,start_page=scan_start,
                        table_title=str(
                            candidate.get("raw_heading")
                            or child.get("raw_label") or ""
                        ),
                    )
                    if source_target else None
                )
                if local_context is not None:
                    # The immutable source locator identifies this physical
                    # page.  Do not let a duplicate leaf ordinal elsewhere in
                    # the report expand or move the candidate scope.
                    scan_scope={
                        **scan_scope,
                        "scan_start_page":scan_start,
                        "scan_end_page":scan_start,
                        "next_note_boundary_page":local_context.get(
                            "next_peer_page"
                        ),
                        "next_note_boundary_bbox":local_context.get(
                            "next_peer_bbox"
                        ) or {},
                        "next_note_reference":"",
                        "next_note_title":local_context.get(
                            "next_peer_title"
                        ) or "",
                        "boundary_status":(
                            "CONFIRMED_PEER_HEADING"
                            if local_context.get("next_peer_page")
                            else "UNRESOLVED"
                        ),
                        "source_target_local_resolution":True,
                    }
                ...
                if local_context is not None:
                    boundary=_source_target_boundary_contract(
                        local_context,start_page=scan_start,
                    )
                    title_bbox=local_context["title_bbox"]
                else:
                ...
                evidence_lines_by_page=lines_by_page
                if (
                    local_context is not None
                    and structure.get("inventory_status")!="COMPLETE"
                ):
                    ...
                    ocr_payload,ocr_audit=ocr_statement_page_group(
                        pdf_path,
                        cache_root=self.repo.registry.db_path.parent / "cache",
                        page_numbers=[scan_start],
                        recovery_stage=(
                            "CHILD_NOTE_INVENTORY_EVIDENCE_RECOVERY"
                        ),
                    )
                    page_payload=dict(ocr_payload.get(scan_start) or {})
                    page=doc[scan_start-1]
                    normalized_words,_,normalized_metadata=(
                        _normalize_ocr_words_to_pdf_points(
                    ...
                    if normalized_metadata is not None and normalized_words:
                        word_dicts=[{
                        ...
                        word_dicts=_repair_compact_ocr_period_words(word_dicts)
                        ocr_lines=_group_lines(word_dicts)
                        peer_bbox=dict(local_context.get("next_peer_bbox") or {})
                        peer_y0=(
                            float(peer_bbox["y0"])
                            if local_context.get("next_peer_page")==scan_start
                            and peer_bbox.get("y0") is not None else None
                        )
                        narrative_y0=_post_total_narrative_boundary_y(
                            ocr_lines,
                            title_bottom=float(title_bbox["y1"]),
                            peer_y0=peer_y0,
                        )
                        recovered_boundary=_source_target_boundary_contract(
                            local_context,start_page=scan_start,
                            narrative_y0=narrative_y0,
                        )
                        recovered=plan_table_structure_candidates(
                            {scan_start:ocr_lines},
                            {scan_start:float(page.rect.width)},
                            start_page=scan_start,end_page=scan_start,
                            title_bbox=title_bbox,
                            note_identity=scan_scope.get("note_identity") or "",
                            table_identity=(
                                candidate.get("raw_heading")
                            ...
                            unit=child.get("unit") or "",
                            boundary=recovered_boundary,
                            candidate_namespace=candidate["candidate_id"],
                        )
                        if recovered.get("inventory_status")=="COMPLETE":
                            recovered=_freeze_certified_ocr_geometry(
                                recovered,
                                page_number=scan_start,
                                ocr_lines=ocr_lines,
                                recovery_audit=ocr_audit,
                            )
                            structure={
                                **recovered,
                                "evidence_source":(
                                    "NATIVE_TITLE_FAST_INDEX_OCR_STRUCTURE_PDF_POINTS"
                                ),
                                "recovery_stage":(
                                    "CANDIDATE_EVIDENCE_RECOVERY"
                                ),
                                "ocr_recovery_audit":{
                                    "ocr_pages":list(
                                        ocr_audit.get("ocr_pages") or []
                                    ),
                                    "ocr_cache_hits":int(
                                        ocr_audit.get("ocr_cache_hits") or 0
                                    ),
                                    "ocr_cache_misses":int(
                                        ocr_audit.get("ocr_cache_misses") or 0
                                    ),
                                    "final_status":ocr_audit.get("final_status"),
                                    "geometry_schema_version":normalized_metadata.get(
                                        "geometry_schema_version"
                                    ),
                                },
                            }
                            evidence_lines_by_page={scan_start:ocr_lines}
                inventory=self._inventory_payload(candidate,scan_scope,structure)
                text="\n".join(
                    str(line.get("text") or "")
                    for line in evidence_lines_by_page.get(scan_start,[])
                )
                ...
                requested=child.get("statement_scope") or "UNKNOWN"; hint=_scope(native_text)
