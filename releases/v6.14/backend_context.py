...
def build_backend_services(paths: dict[str, Path]) -> BackendServices:
    ...
    pdf=PdfService(pdf_repo,paths);job=JobService(job_repo);capture=CaptureService(capture_repo,paths);registry_service=RegistryService(registry,synchronizer,paths)
