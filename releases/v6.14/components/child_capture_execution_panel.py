...
def render_child_capture_execution_panel(
    ...
    *,
    ...
    feedback_state_key = f"{key_prefix}_submission_feedback"
    submission_feedback = st_obj.session_state.pop(feedback_state_key, None)
    if submission_feedback:
        st_obj.success(str(submission_feedback))
    ...
    selected_policy = (
        ...
        else CaptureScopePolicy.PRIMARY_WITH_CONTINUATIONS.value
    )
    ...
    # ---- execute button ----
    all_submitted = bool(state.get("all_plans_submitted"))
    all_terminal = bool(state.get("all_terminal"))
    execution_in_progress = all_submitted and not all_terminal
    replay_requested = all_submitted and all_terminal
    if execution_in_progress:
        st_obj.info("当前 Capture Plan 已提交，抓取作业仍在执行；请使用下方进度刷新。")
    elif replay_requested:
        st_obj.info(
            "当前 Capture Plan 已有终态执行记录；再次提交将创建新的研究批次和抓取作业，"
            "不会覆盖历史 Capture。"
        )
    if st_obj.button(
        "重新抓取当前逻辑表" if replay_requested else "确认逻辑表并抓取",
        type="primary",
        disabled=not can_execute or execution_in_progress,
