...
class CaptureDecisionReducer:
    ...
    def _derive_issue_codes(
        *,
        ...
        # Hierarchy graph validation
        hierarchy_validation = (evidence.get("stats") or {}).get("hierarchy_graph_validation") or {}
        if str(hierarchy_validation.get("status") or "VALID").upper() == "REVIEW_REQUIRED":
            blocking.append("REVIEW_REQUIRED_SOURCE_IDENTITY")
            for reason in hierarchy_validation.get("review_reasons") or []:
