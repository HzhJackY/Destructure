...
# DATA_HOME_LAYOUT_SCHEMA_VERSION describes the persistent on-disk layout. It
# intentionally evolves independently from APP_VERSION (the running code).
DATA_HOME_LAYOUT_SCHEMA_VERSION = "6.10"
SCHEMA_VERSION = DATA_HOME_LAYOUT_SCHEMA_VERSION
...
def ensure_data_home(data_home: Path, bundled_rules: Path) -> dict[str, Path]:
    ...
    if manifest.exists():
        ...
        data = {}
    data["data_schema_version"] = DATA_HOME_LAYOUT_SCHEMA_VERSION
    # Preserve the creator of an existing DATA_HOME. A newly created manifest
    # records the current application identity from the single version source.
    data.setdefault("created_by", APP_VERSION)
    data["last_opened_by"] = APP_VERSION
