...
def _result_dataclass(
    ...
    columns = [
        TableColumn(
            ...
            measure=(
                str(c.get("measure")).strip()
                if c.get("measure") not in [None, ""] else None
            ),
            period_kind=(
                str(c.get("period_kind")).strip()
                if c.get("period_kind") not in [None, ""] else None
            ),
            source_period_label=c.get("source_period_label"),
            period_year=c.get("period_year"),
            period_month=c.get("period_month"),
            period_day=c.get("period_day"),
            period_precision=c.get("period_precision"),
            period_date=c.get("period_date"),
            period_identity=c.get("period_identity"),
            period_normalization_evidence=c.get(
    ...
    for r in result.get("rows") or []:
        cells = [
            TableCell(
                ...
                cell_role=str(c.get("cell_role") or "NUMERIC"),
                context_source_page=c.get("context_source_page"),
        ...
        rows.append(TableRow(
            ...
            row_role=str(r.get("row_role") or r.get("row_type") or "DETAIL"),
            row_item_raw=r.get("row_item_raw"),
            row_item_normalized=r.get("row_item_normalized"),
            label_derivation=str(r.get("label_derivation") or "EXPLICIT_TEXT"),
            derivation_method=r.get("derivation_method"),
            derived_from_rows=r.get("derived_from_rows"),
            derivation_evidence=r.get("derivation_evidence"),
            bbox=r.get("bbox"),
            observation_type=str(r.get("observation_type") or "SOURCE_OBSERVATION"),
            derived_status=str(r.get("derived_status") or ""),
            container_id=r.get("container_id"),
            table_block_id=r.get("table_block_id"),
            block_order=(
                int(r.get("block_order"))
                if r.get("block_order") not in (None, "")
                else None
            ),
            classification_axis=str(r.get("classification_axis") or "UNRESOLVED"),
            block_role=str(r.get("block_role") or "UNRESOLVED"),
            block_terminal_type=str(r.get("block_terminal_type") or "UNRESOLVED"),
            excluded_from_table_logic=bool(r.get("excluded_from_table_logic")),
        ))
    ...
    return TableCaptureResult(
        ...
        boundary_status_source=result.get("boundary_status_source"),
    )
...
def rematerialize_official_capture(run_dir: Path) -> dict[str, int]:
    ...
    # Human row-noise decisions never mutate machine ``rows``.  They only
    # affect this official, merge-facing materialization in the revision that
    # contains the auditable decision record.
    from capture_library import human_layout_noise_orders
    noise_orders = human_layout_noise_orders(result)
    ...
    if noise_orders and "row_order" in official_long:
        official_orders = pd.to_numeric(official_long["row_order"], errors="coerce")
        noise_long = official_long[official_orders.isin(noise_orders)].copy()
        official_long = official_long[~official_orders.isin(noise_orders)].copy()
        if not noise_long.empty:
            noise_long["official_exclusion_reason"] = "HUMAN_LAYOUT_NOISE"
            excluded = pd.concat([excluded, noise_long], ignore_index=True)
    if noise_orders and "row_order" in official_wide:
        official_orders = pd.to_numeric(official_wide["row_order"], errors="coerce")
        official_wide = official_wide[~official_orders.isin(noise_orders)].copy()
    ...
    structure_audit = subtotal_validation(official_long)
    structure_audit.to_csv(
        run_dir / "table_structure_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    with pd.ExcelWriter(run_dir / "table_capture.xlsx", engine="openpyxl") as writer:
        ...
        structure_audit.to_excel(writer, sheet_name="structure_audit", index=False)
...
def apply_header_dimension_review(
    ...
    for machine in machine_cols:
        ...
        measure = str(
            edited.get("measure")
            if edited.get("measure") is not None
            else machine.get("measure") or ""
        ).strip()
        period_label = str(
            edited.get("period_label")
            if edited.get("period_label") is not None
            else machine.get("period_label") or year
        ).strip() or year
        period = normalize_period_fields(
            source_period_label=(
                edited.get("source_period_label")
                or machine.get("source_period_label")
                or machine.get("period_label")
            ),
            period_label=period_label,
            year=year,
        )
        if not period.get("period_identity"):
            raise ValueError(
                f"col{ordinal} 的期间无法生成稳定 period_identity。"
            )
        if period.get("period_year") is not None:
            year = str(period["period_year"])
        period_label = str(period.get("period_label") or period_label)
