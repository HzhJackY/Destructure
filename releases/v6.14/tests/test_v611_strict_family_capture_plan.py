...
def test_certified_inventory_change_creates_a_new_immutable_plan_id(tmp_path: Path):
    ...
    original = service._strict_links_to_plans([_link(9), _link(10)], **kwargs)[0]
    expanded = service._strict_links_to_plans(
        [_link(9), _link(10), _link(11)], **kwargs,
    )[0]
    reordered = service._strict_links_to_plans(
        [_link(11), _link(10), _link(9)], **kwargs,
    )[0]

    assert original["plan_id"] != expanded["plan_id"]
    assert reordered["plan_id"] == expanded["plan_id"]
