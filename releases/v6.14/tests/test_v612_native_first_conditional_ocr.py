...
def test_v614_release_identity_is_authoritative():
    assert APP_VERSION == "v6.14"
...
def test_full_index_cache_key_includes_tesseract_layout_mode():
    base = dict(
        ocr_mode="selected",
        language="chi_sim+eng",
        dpi=400,
    ...
    assert fast_index.cache_key(**base, ocr_psm=4) != fast_index.cache_key(
        **base,
        ocr_psm=6,
    )


def test_red_stamp_mask_preserves_wide_header_band_and_removes_compact_patch():
    import numpy as np

    image = np.full((20, 100, 3), 255, dtype=np.uint8)
    image[2:5, 5:95] = [152, 117, 0]  # wide gold/brown table header
    image[10:14, 10:18] = [220, 20, 20]  # compact red stamp fragment

    masked = fast_index._red_stamp_mask_preserving_wide_bands(image)

    assert np.array_equal(masked[3, 50], [152, 117, 0])
    assert np.array_equal(masked[11, 12], [255, 255, 255])
