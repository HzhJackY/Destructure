...
def test_incomplete_inventory_without_unresolved_table_still_opens_review_case(
    tmp_path:Path,
):
    _,repository=_repository(tmp_path)
    candidate=_seed_candidate(repository)
    ...
    inventory["inventory_status"]="INCOMPLETE"
    inventory["evidence"]={
        "issue_codes":["TABLE_SEGMENT_SIGNATURE_INCOMPLETE"],
    }
    ...
    assert persisted is not None
    assert persisted["inventory_status"]=="INCOMPLETE"
    assert persisted["unresolved_table_count"]==0
    cases=repository.unresolved_inventory_cases(
        anchor_child_id=candidate["anchor_child_id"],
        candidate_ids=[candidate["candidate_id"]],
    )
    assert len(cases)==1
    assert cases[0]["reason"]=="PERSISTED_UNRESOLVED_NOTE_TABLE_INVENTORY"


def test_cached_incomplete_inventory_without_case_is_backfilled_for_assignment(
    tmp_path:Path,
):
    registry,repository=_repository(tmp_path)
    candidate=_seed_candidate(repository)
    ...
    with registry.connect() as conn:
        conn.execute(
            """UPDATE child_note_table_inventories
               SET inventory_status='INCOMPLETE'
               WHERE candidate_id=?""",
    ...
    cases=repository.ensure_unresolved_inventory_cases(
        anchor_child_id=candidate["anchor_child_id"],
        candidate_ids=[candidate["candidate_id"]],
    )

    assert len(cases)==1
    assert cases[0]["candidate_id"]==candidate["candidate_id"]
...
def test_certified_links_for_anchor_requires_and_matches_full_identity(
    tmp_path:Path,
):
    _,repository=_repository(tmp_path)
    _seed_and_certify_inventory(repository)
    certified=repository.certify(
        {
            **_link(),
            "logical_table_candidate_id":"LTABLE_CANDIDATE_13",
            "research_definition_id":"FINANCIAL_INVESTMENT_V1",
            "definition_version":"FINANCIAL_INVESTMENT_V1",
        },
        reviewer="reviewer",method="CERTIFY_LOGICAL_TABLE_CANDIDATE",
    )
    restored=repository.certified_links_for_anchor(
        "ANCHOR_FIXTURE",
        table_family_id="financial_investment",
        statement_scope="CONSOLIDATED",
        research_definition_id="FINANCIAL_INVESTMENT_V1",
    ...
    assert [row["certified_link_id"] for row in restored]==[
        certified["certified_link_id"]
    ]
    assert repository.certified_links_for_anchor(
        "ANCHOR_FIXTURE",
    ...
    with pytest.raises(
        ValueError,match="CERTIFIED_LINK_RESTORE_IDENTITY_REQUIRED",
    ):
        repository.certified_links_for_anchor(
            "ANCHOR_FIXTURE",
            table_family_id="financial_investment",
            statement_scope="CONSOLIDATED",
...
def test_certified_links_restore_across_append_only_physical_anchor_revision(
    tmp_path:Path,
):
    registry,repository=_repository(tmp_path)
    occurrences=DiscoveryRegistry(registry)
    owner=occurrences.save_occurrence({
        "occurrence_id":"ANCHOR_FIXTURE",
        "pdf_id":"C:/reports/filing.pdf",
        "company":"测试保险",
        "report_year":"2025",
        ...
        "statement_pdf_page_index":100,
        "child_rows":[],
    })
    occurrences.adjudicate_anchor(
        owner["occurrence_id"],label="ACCEPTED",chosen_scope="CONSOLIDATED",
    )
    _seed_and_certify_inventory(repository)
    certified=repository.certify(
        {
    ...
    restored=repository.certified_links_for_physical_anchor(
        {
            "occurrence_id":"FRESH_APPEND_ONLY",
            "pdf_id":"C:/REPORTS/FILING.PDF",
            "report_year":"2025",
            "statement_type":"BALANCE_SHEET",
            "statement_pdf_page_index":100,
    ...
    assert repository.certified_links_for_physical_anchor(
        {
            "pdf_id":"C:/reports/filing.pdf",
            "report_year":"2025",
            "statement_type":"BALANCE_SHEET",
            "statement_pdf_page_index":101,
        },
        table_family_id="financial_investment",
        statement_scope="CONSOLIDATED",
        research_definition_id="FINANCIAL_INVESTMENT_V1",
...
def test_certified_link_revocation_is_owner_audited_and_idempotent(tmp_path:Path):
    registry,repository=_repository(tmp_path)
    _seed_and_certify_inventory(repository)
    certified=repository.certify(
        {**_link(),"logical_table_candidate_id":"LTABLE_CANDIDATE_13"},
    ...
    revoked=repository.revoke_certified_link(
        certified["certified_link_id"],
        reviewer="agent",
        reason_code="COMPARATIVE_ONLY_PERIOD_RECLASSIFICATION",
        reason="当前期间为不适用，原链接只属于比较期。",
        evidence={"current_period_raw":"-","comparative_period_raw":"26,560"},
    )
    repeated=repository.revoke_certified_link(
        certified["certified_link_id"],
        reviewer="agent",
        reason_code="COMPARATIVE_ONLY_PERIOD_RECLASSIFICATION",
        reason="幂等重试。",
    ...
    assert repeated["certification_status"]=="REVOKED"
    with pytest.raises(PermissionError,match="CERTIFIED_CHILD_TABLE_LINK_REQUIRED"):
        repository.certified_target(certified["certified_link_id"])
    with registry.connect() as conn:
        link_status=conn.execute(
            "SELECT certification_status FROM certified_child_table_links WHERE certified_link_id=?",
            (certified["certified_link_id"],),
        ).fetchone()[0]
        segment_statuses={row[0] for row in conn.execute(
            "SELECT certification_status FROM certified_child_table_segments WHERE certified_link_id=?",
            (certified["certified_link_id"],),
        ).fetchall()}
        audits=conn.execute(
            """SELECT action,evidence_json FROM child_mapping_review_records
               WHERE anchor_child_id=? AND action='REVOKE_CERTIFIED_CHILD_TABLE_LINK'""",
            (certified["anchor_child_id"],),
        ).fetchall()
    assert link_status=="REVOKED"
    assert segment_statuses=={"REVOKED"}
    assert len(audits)==1
    assert "COMPARATIVE_ONLY_PERIOD_RECLASSIFICATION" in audits[0][1]
...
def test_human_adjudicated_supplementary_can_omit_period_signature(
    tmp_path:Path,
):
    _,repository=_repository(tmp_path)
    candidate=_seed_candidate(repository)
    ...
    logical_decisions=[]
    segment_decisions=[]
    for logical in inventory["logical_tables"]:
        classification=logical["classification"]
        logical_id=logical["logical_table_candidate_id"]
        logical_decisions.append({
            "logical_table_candidate_id":logical_id,
        ...
        for segment in logical["segments"]:
            ...
            segment_decisions.append({
                "segment_candidate_id":segment["segment_candidate_id"],
                "logical_table_candidate_id":logical_id,
                "classification":(
                    "CONTINUATION_SEGMENT" if parent else classification
                ),
                "continuation_of_segment_candidate_id":parent,
            })
    repository.adjudicate_inventory_case(
        case["resolution_case_id"],reviewer="reviewer",
        reason="核对既有候选的主表、补充表和续表语义",
        decisions={
            "logical_tables":logical_decisions,
    ...
    assert assignment["decisions"][0]["status"]=="AUTO_CERTIFIED"
    assert len(assignment["certified_links"])==2
