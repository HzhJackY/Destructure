...
def test_certified_v3_context_preserves_period_group_evidence() -> None:
    segment = _segment(["2025"] * 4)
    segment["period_signature"] = {
        "contract_version": 3,
        "period_labels": ["2025"] * 4,
        "period_kinds": ["ABSOLUTE_YEAR"] * 4,
        "year_labels": ["2025"] * 4,
        "lane_group_ids": ["PERIOD_GROUP_1"] * 4,
        "column_groups": [{
            "period": "2025",
            "period_anchor_bbox": {"x0": 300, "y0": 20, "x1": 700, "y1": 32},
            "period_group_bbox": {"x0": 250, "y0": 20, "x1": 740, "y1": 60},
            "period_header_row_band": {"y0": 20, "y1": 32},
            "child_header_row_band": {"y0": 40, "y1": 60},
            "consumed_spans": [{
                "line_index": 0,
                "word_indices": [0],
                "text": "2025年",
                "bbox": {"x0": 300, "y0": 20, "x1": 700, "y1": 32},
                "method": "MAXIMAL_CONTIGUOUS_PERIOD_SPAN",
            }],
            "column_group_id": "PERIOD_GROUP_1",
            "confidence": 1.0,
            "evidence": {"mapping_mode": "MULTIROW_CONTIGUOUS_COLUMN_GROUP"},
        }],
    }
    segment["header_signature"] = {
        "contract_version": 3,
        "leaf_count": 4,
        "labels": MEASURES,
        "measure_kinds": ["OTHER"] * 4,
        "leaf_bboxes": [
            {"x0": center - 20, "y0": 40, "x1": center + 20, "y1": 60}
            for center in CENTERS
        ],
        "header_bbox": {"x0": 300, "y0": 20, "x1": 700, "y1": 60},
    }
    segment["amount_lane_signature"] = {
        "contract_version": 3,
        "lane_count": 4,
        "anchor_ratios": [center / PAGE_WIDTH for center in CENTERS],
        "source_column_ordinals": [2, 3, 4, 5],
    ...
    header, _arbitration = spatial._certified_header(context)

    assert context["column_topology_contract_version"] == 3
    assert context["lane_group_ids"] == ["PERIOD_GROUP_1"] * 4
    assert header["period_groups"][0]["column_group_id"] == "PERIOD_GROUP_1"


def test_certified_v4_period_signature_accepts_v3_geometry() -> None:
    segment = _segment(["2025年12月31日"] * 4)
    segment["period_signature"] = {
        "contract_version": 4,
        "period_labels": ["2025年12月31日"] * 4,
        "period_kinds": ["ABSOLUTE_DATE"] * 4,
        "year_labels": ["2025"] * 4,
        "lane_group_ids": ["PERIOD_GROUP_1"] * 4,
        "periods": [{"period_identity": "DATE:2025-12-31"}] * 4,
        "column_groups": [{
            "period": "2025年12月31日",
            "period_anchor_bbox": {"x0": 300, "y0": 20, "x1": 700, "y1": 32},
            "period_group_bbox": {"x0": 250, "y0": 20, "x1": 740, "y1": 60},
            "period_header_row_band": {"y0": 20, "y1": 32},
            "child_header_row_band": {"y0": 40, "y1": 60},
            "consumed_spans": [{"text": "2025年12月31日"}],
            "column_group_id": "PERIOD_GROUP_1",
            "confidence": 1.0,
            "evidence": {},
        }],
    ...
    segment["amount_lane_signature"]["contract_version"] = 3

    context = spatial._certified_column_context(
        [segment], page_number=1, lines=_body_lines(), page_width=PAGE_WIDTH,
    )

    assert context["column_topology_contract_version"] == 4
    assert context["period_labels"] == ["2025年12月31日"] * 4
