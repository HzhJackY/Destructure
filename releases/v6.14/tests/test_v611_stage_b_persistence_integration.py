...
def test_explicit_terminal_replay_creates_fresh_session_and_lineage(tmp_path):
    registry,guided,svc=_service(tmp_path)
    pdf_path=_pdf(tmp_path)
    kwargs={
        "display_name":"family_fixture",
    ...
    first=svc.create_execution_batch(**kwargs)
    assert first["all_plans_submitted"] is True
    assert first["all_terminal"] is False
    assert len(guided.calls)==1
    with pytest.raises(RuntimeError,match="STAGE_B_EXECUTION_STILL_ACTIVE"):
        svc.create_execution_batch(**kwargs,create_new_attempt=True)
    assert len(guided.calls)==1
    with registry.connect() as conn:
        conn.execute(
            "UPDATE jobs SET status='SUCCESS',progress=1 WHERE batch_id=?",
            (first["batch_ids"][0],),
        )

    terminal=svc.restore_execution(first["session_key"])
    assert terminal["all_plans_submitted"] is True
    assert terminal["all_terminal"] is True

    idempotent=svc.create_execution_batch(**kwargs)
    assert idempotent["session_key"]==first["session_key"]
    assert idempotent["research_batch_id"]==first["research_batch_id"]
    assert len(guided.calls)==1

    replay=svc.create_execution_batch(**kwargs,create_new_attempt=True)
    assert replay["session_key"]!=first["session_key"]
    assert replay["session_key"].startswith(
        svc.execution_session_key(
            display_name="family_fixture",
            research_definition=kwargs["research_definition"],
            scope="CONSOLIDATED",
        )+"__V"
    )
    assert "__R" in replay["session_key"]
    assert replay["research_batch_id"]!=first["research_batch_id"]
    assert replay["batch_ids"]!=first["batch_ids"]
    assert replay["all_plans_submitted"] is True
    assert len(guided.calls)==2
    assert svc.restore_execution(first["session_key"])["batch_ids"]==first["batch_ids"]
    with registry.connect() as conn:
        sessions=conn.execute(
            "SELECT COUNT(*) n FROM stage_b_execution_sessions"
        ).fetchone()["n"]
        research_batches=conn.execute(
            "SELECT COUNT(*) n FROM research_batches"
        ).fetchone()["n"]
    assert sessions==2
    assert research_batches==2
...
def test_stage_b_ui_has_one_db_restored_panel_and_one_workspace_route():
    ...
    assert "CaptureScopePolicy.PRIMARY_WITH_CONTINUATIONS.value" in panel
    assert "仅抓取主逻辑表的首个已认证物理片段" in panel
