...
def test_app_creation_ui_offers_note_ordinal_ordering():
    ...
    controls_source = (ROOT / "merge_order_controls_ui.py").read_text(encoding="utf-8")
    assert "render_merge_order_controls(" in app_source
    assert "按年份附注号排序" in controls_source
    assert "merge_order_policy_selector" in controls_source
    assert "merge_order_reference_year" in controls_source
    assert "order_selection.reference_capture_run_id" in app_source
    assert "order_selection.order_policy" in app_source
    assert "order_selection.reference_report_year" in app_source
