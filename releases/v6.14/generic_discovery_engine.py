...
from generic_discovery import (
    discover as statement_discover,
    assemble_statement_occurrences,
    normalize_company,
    _infer_statement_scope,
)
from conditional_statement_ocr import directory_statement_hints, ocr_statement_page_group
...
class GenericDiscoveryService:
    def __init__(self, definition_service, cache_root: Path):
        ...
        self.portfolio_resolver = InvestmentPortfolioTopologyResolver()
    ...
    def discover(self, *, pdf_path: Path, definition_id: str, company: str = "", report_year: str = "", filing_type: str = "ANNUAL_REPORT") -> dict[str, Any]:
        ...
        if str(definition.get("status") or "") != "ACTIVE":
            raise PermissionError("RESEARCH_DEFINITION_NOT_ACTIVE")
        ...
        for family_id in payload.get("table_families", []):
            ...
            strategy = (
                (payload.get("family_strategy_overrides") or {}).get(family_id)
                or family["discovery_strategy"]
            )
            members = self.definitions.members(family_id)
            if strategy == "DIRECT_PORTFOLIO_TABLES":
                candidates, audit = self._direct_portfolio_tables(
                    pdf_path, family, members, company, report_year, filing_type,
                )
                all_candidates.extend(candidates)
                all_occurrences.extend(self.structure_parser.parse(
                    candidates,
                    strategy=strategy,
                    family_id=family_id,
                    display_name=family["display_name"],
                ))
                discovery_audits.append({"family_id": family_id, **audit})
                if not candidates:
                    failures.append({
                        "family_id": family_id,
                        "failure_reason": audit.get("final_status", "NO_DIRECT_PORTFOLIO_TABLE"),
                        "portfolio_topology_audit": audit,
                    })
            elif strategy == "DIRECT_NOTE_TABLE_FAMILY":
            ...
            elif strategy == "DIRECT_MAIN_STATEMENT_TABLE":
                candidates = self._direct_main_statement_table(
                    pdf_path, family, members, company, report_year, filing_type,
                )
                all_candidates.extend(candidates)
                all_occurrences.extend(self.structure_parser.parse(candidates, strategy=strategy, family_id=family_id, display_name=family["display_name"]))
    ...
    def _direct_portfolio_tables(
        ...
        # Native text is always the first pass.  Scanned-document conditional
        # OCR is restricted to pages whose Native title/axis/unit/total
        # identity is already strong but whose image-backed numeric layer is
        # absent.  The same Fast Index page cache remains the sole OCR owner.
        index = build_text_index(pdf_path, self.cache_root, ocr_mode="off")
        candidates, audit = self.portfolio_resolver.resolve(
            pdf_path=pdf_path,
            index=index,
            members=members,
            company=company,
        ...
        recovery_pages = self.portfolio_resolver.evidence_recovery_pages(index)
        if not recovery_pages:
            return candidates, audit

        ocr_records: list[Any] = []
        _, ocr_audit = ocr_statement_page_group(
            pdf_path,
            cache_root=self.cache_root,
            page_numbers=recovery_pages,
            recovery_stage="PORTFOLIO_CANDIDATE_EVIDENCE_RECOVERY",
            record_sink=ocr_records,
        )
        native_by_page = {int(record.page_number): record for record in index}
        combined_index: list[TextIndexRecord] = []
        for ocr_record in fast_index_to_text_index(ocr_records):
            page = int(ocr_record.page_number)
            native = native_by_page.get(page)
            native_text = str(native.text or "") if native else ""
            recovered_period_headers = ocr_period_headers_from_words(
                getattr(ocr_record, "ocr_words", None) or []
            )
            period_geometry_text = "\n".join(recovered_period_headers)
            combined = TextIndexRecord(
                page_number=page,
                text=(
                    native_text
                    + "\n"
                    + str(ocr_record.text or "")
                    + ("\n" + period_geometry_text if period_geometry_text else "")
                ),
                heading=str(native.heading or "") if native else str(ocr_record.heading or ""),
                section=str(native.section or "") if native else str(ocr_record.section or ""),
                note_numbers=tuple(native.note_numbers or ()) if native else tuple(ocr_record.note_numbers or ()),
            )
            for attr in ("ocr_rows", "ocr_words", "ocr_used", "ocr_error", "page"):
                if hasattr(ocr_record, attr):
                    object.__setattr__(combined, attr, getattr(ocr_record, attr))
            combined_index.append(combined)
        recovered, recovered_audit = self.portfolio_resolver.resolve(
            pdf_path=pdf_path,
        ...
        for row in recovered:
            ...
            evidence.update({
                "ocr_used": True,
                "geometry_evidence_mode": "HYBRID_NATIVE_IDENTITY_OCR_VALUES",
                "identity_source": "NATIVE_PDF_TEXT",
                "value_source": "FAST_INDEX_OCR_WORDS",
                "recovery_stage": "PORTFOLIO_CANDIDATE_EVIDENCE_RECOVERY",
                "ocr_recovery_audit": dict(ocr_audit),
                "ocr_period_headers_from_geometry": recovered_period_headers,
            })
            row["locator_method"] = "DIRECT_PORTFOLIO_NATIVE_IDENTITY_OCR_VALUES"
        return recovered, {
            **audit,
            **recovered_audit,
    ...
    def _direct_main_statement_table(
        self,
        pdf_path: Path,
        family: dict[str, Any],
        members: list[dict[str, Any]],
        ...
        """Locate a Registry-declared primary statement without a note detour.

        The result is still an Anchor candidate and later materialises a
        ``CertifiedChildTableLink``.  It only changes the physical target: the
        reviewed whole table is on the certified main-statement page itself.
        """
        family_payload = dict(family.get("payload") or {})
        preferred_type = str((family_payload.get("preferred_statement_types") or ["BALANCE_SHEET"])[0])
        preferred_scope = str(family_payload.get("preferred_scope") or "CONSOLIDATED")
        audit: dict[str, Any] = {"statement_index_source": "FAST_INDEX_NATIVE_ONLY", "ocr_triggered": False}
        try:
            from document_index_profile import fast_index_profile_kwargs
            fast_records, fast_meta = build_fast_index(
                Path(pdf_path), self.cache_root, **fast_index_profile_kwargs(ocr_mode="off")
            )
            index = fast_index_to_text_index(fast_records)
            audit["statement_index_cache_hit"] = bool(fast_meta.get("cache_hit"))
        except Exception as exc:
            index = build_text_index(pdf_path, self.cache_root, ocr_mode="off")
            audit["statement_index_source"] = "NATIVE_TEXT_FALLBACK"
            audit["statement_index_error"] = type(exc).__name__
        pages = set(locate_primary_statements(index).get(preferred_type, []))
        records_by_page = {rec.page_number: rec for rec in index}
        pages_by_scope: dict[str, list[int]] = {}
        for page in sorted(pages):
            rec = records_by_page.get(page)
            if not rec:
                continue
            scope = _infer_statement_scope(
                rec.text, preferred_type, page_number=page, statement_pages=sorted(pages),
            )
            if preferred_scope not in {"", "BOTH", "UNKNOWN"} and scope not in {preferred_scope, "COMBINED_CONSOLIDATED_AND_PARENT"}:
                continue
            pages_by_scope.setdefault(scope, []).append(page)
        found: list[dict[str, Any]] = []
        for scope, scoped_pages in pages_by_scope.items():
            start_page = min(scoped_pages)
            end_page = start_page
            for page in sorted(scoped_pages):
                if page == end_page + 1:
                    end_page = page
                    ...
                    break
            rec = records_by_page[start_page]
            table_text = "\n".join(str(records_by_page[page].text or "") for page in scoped_pages if page <= end_page)
            top = "\n".join(str(rec.text or "").splitlines()[:32])
            for member in members:
                if member.get("member_role") != "DIRECT_MAIN_STATEMENT_TABLE":
                    continue
                titles = [str(member.get("display_name") or ""), *list((member.get("payload") or {}).get("aliases") or [])]
                matched_title = next((title for title in titles if normalize_text(title) in normalize_text(top)), member.get("display_name") or family["display_name"])
                row_hits = sum(1 for token in (member.get("payload") or {}).get("row_signatures", []) if normalize_text(token) in normalize_text(table_text))
                confidence = round(min(0.99, 0.86 + min(0.10, row_hits * 0.03)), 2)
                found.append({
                    "discovery_id": "DMS_" + uuid.uuid4().hex,
                    "company": company, "normalized_company": normalize_company(company),
                    "report_year": str(report_year), "filing_type": filing_type,
                    "statement_type": preferred_type, "scope": scope,
                    "display_name": family["display_name"], "table_family": family["family_id"],
                    "definition_version": family["definition_version"],
                    "statement_item": member["display_name"], "member_table": member["member_id"],
                    "member_display_name": member["display_name"],
                    "matched_title": matched_title, "certified_heading": matched_title,
                    "source_table_title": matched_title,
                    "statement_pdf_page_index": start_page,
                    "candidate_note_pdf_page_index": start_page,
                    "candidate_note_pages": list(range(start_page, end_page + 1)),
                    "locator_method": "DIRECT_PRIMARY_STATEMENT_TITLE_AND_SIGNATURE",
                    "confidence": confidence, "status": "NEEDS_REVIEW",
                    "note_reference": "", "note_reference_status": "NOT_APPLICABLE",
                    "classification_axis": (member.get("payload") or {}).get("classification_axis") or "STATEMENT_TABLE",
                    "direct_main_statement": True,
                    "direct_end_page": end_page,
                    "evidence": {
                        "strategy": "DIRECT_MAIN_STATEMENT_TABLE",
                        "registry_family": family["family_id"],
                        "statement_index": audit,
                        "matched_title": matched_title,
                        "row_signature_hits": row_hits,
                        "source_page_text_chars": len(table_text),
                        "statement_page_range": [start_page, end_page],
                    },
                })
        return found
