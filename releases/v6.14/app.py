...
def header_dimension_review_widget(run_dir: Path, key_prefix: str) -> None:
    ...
    if not machine_df.empty:
        show_cols = [c for c in [
            "ordinal","source_column_index","header_raw","source_period_label",
            "period_label","period_year","period_month","period_day",
            "period_precision","period_date","period_identity","period_kind",
    ...
    for index, row in edit_df.iterrows():
        ...
        for field in (
            "source_period_label", "period_label", "period_year",
            "period_month", "period_day", "period_precision",
            "period_date", "period_identity", "period_kind",
        ):
            if field not in edit_df.columns or pd.isna(edit_df.at[index, field]):
                edit_df.at[index, field] = period.get(field)

    for col in [
        "year","source_period_label","period_label","period_precision",
        "period_date","period_identity","period_kind","scope","measure","header_raw",
    ]:
        if col in edit_df:
    ...
    edit_cols=[c for c in [
        "ordinal","source_column_index","year","source_period_label",
        "period_label","period_year","period_month","period_day",
        "period_precision","period_date","period_identity","period_kind",
        "scope","measure","restated","header_raw"
...
# -----------------------------------------------------------------------------
    ...
    st.caption("内置 Registry 由发行版维护且只读；用户 Registry 先保存为 DATA_HOME 草稿，经校验后显式启用，随后复用同一 Discovery → CertifiedChildTableLink → Capture 链路。")
    service = BACKEND.research_definition_service
    tabs = st.tabs(["Research Definitions", "Table Families", "用户 Registry 草稿", "Metric ↔ Family", "Discovery Strategies", "Knowledge Library", "导入/导出"])
    with tabs[0]:
        ...
        if definitions:
            selected = st.selectbox("选择已有版本", [x["definition_id"] for x in definitions], key="v67_selected_definition")
            st.caption("新建或变更用户 Registry 请使用“用户 Registry 草稿”：草稿通过校验后才会原子启用，不会把不可用版本带入批次发现。")
            if not service.is_builtin_definition(selected) and st.button("归档选中的用户 Definition（不影响历史批次）", key="v67_archive_definition"):
                try:
                    service.archive_definition(selected, actor="STREAMLIT")
                    st.success(f"已归档：{selected}")
                    st.rerun()
                except Exception as exc: st.exception(exc)
            elif service.is_builtin_definition(selected):
                st.info("内置 Definition 只读，不可归档。")
    with tabs[1]:
        families = service.families()
        st.dataframe(pd.DataFrame([{**{k: row.get(k) for k in ("family_id", "display_name", "definition_version", "discovery_strategy")}, "source": "内置" if service.is_builtin_family(row["family_id"]) else "用户", "members": len(service.members(row["family_id"]))} for row in families]), use_container_width=True, hide_index=True)
        if families:
            ...
            if not service.is_builtin_family(family_id) and st.button("归档当前用户 Family（不影响历史证据）", key="v67_archive_family"):
                try:
                    service.archive_family(family_id, actor="STREAMLIT")
                    st.success(f"已归档：{family_id}")
                    st.rerun()
                except Exception as exc: st.exception(exc)
            elif service.is_builtin_family(family_id):
                st.info("内置 Family 与 Member 只读，不可归档或覆盖。")
    with tabs[2]:
        template = {
            "family": {"family_id": "custom_direct_table", "display_name": "自定义披露表", "definition_version": "CUSTOM_DIRECT_TABLE_V1", "discovery_strategy": "DIRECT_NOTE_TABLE_FAMILY", "preferred_statement_types": ["NOTE_SECTION"], "preferred_scope": "CONSOLIDATED"},
            "members": [{"member_id": "custom_direct_table_member", "display_name": "自定义披露表", "member_role": "DIRECT_DISCLOSURE_TABLE", "required": True, "canonical_order": 1, "aliases": [], "row_signatures": [], "column_signatures": []}],
            "definition": {"definition_id": "CUSTOM_DIRECT_TABLE_V1", "display_name": "自定义披露表", "definition_version": "CUSTOM_DIRECT_TABLE_V1", "table_families": ["custom_direct_table"], "research_scope": {"core_members": ["custom_direct_table_member"], "optional_members": [], "excluded_members": []}},
        }
        raw_bundle = st.text_area("用户 Registry Bundle（JSON）", value=json.dumps(template, ensure_ascii=False, indent=2), key="v613_user_registry_bundle", height=420)
        st.caption("保存草稿不会进入批次、Discovery 或知识包下拉；必须先查看校验结果，再点击“启用”。")
        if st.button("保存为 DATA_HOME 草稿", key="v613_create_registry_draft"):
            try:
                draft = service.create_user_draft(json.loads(raw_bundle), actor="STREAMLIT")
                st.success(f"草稿已保存：{draft['draft_id']}；校验：{'通过' if draft['validation'].get('valid') else '待修复'}")
                st.rerun()
            except Exception as exc: st.exception(exc)
        drafts = service.user_drafts()
        if drafts:
            st.dataframe(pd.DataFrame([{k: row.get(k) for k in ("draft_id", "display_name", "status", "activated_definition_id", "updated_at")} | {"valid": bool((row.get("validation") or {}).get("valid")), "errors": "；".join((row.get("validation") or {}).get("errors") or [])} for row in drafts]), use_container_width=True, hide_index=True)
            draft_id = st.selectbox("选择用户 Registry 草稿", [row["draft_id"] for row in drafts], key="v613_registry_draft_select")
            selected_draft = service.user_draft(draft_id)
            if selected_draft:
                st.code(json.dumps(selected_draft["payload"], ensure_ascii=False, indent=2), language="json")
                c1, c2, c3 = st.columns(3)
                if c1.button("重新校验", key="v613_validate_registry_draft"):
                    try:
                        report = service.validate_user_draft(draft_id, actor="STREAMLIT")
                        (st.success if report["valid"] else st.error)("校验通过" if report["valid"] else "校验失败：" + "；".join(report["errors"]))
                    except Exception as exc: st.exception(exc)
                if c2.button("启用为正式用户 Registry", key="v613_activate_registry_draft", disabled=selected_draft["status"] != "DRAFT"):
                    try:
                        definition = service.activate_user_draft(draft_id, actor="STREAMLIT")
                        st.success(f"已启用：{definition['definition_id']}；现在才会出现在批次/Discovery 选择中。")
                        st.rerun()
                    except Exception as exc: st.exception(exc)
                if c3.button("归档草稿", key="v613_archive_registry_draft", disabled=selected_draft["status"] == "ACTIVE"):
                    try:
                        service.archive_user_draft(draft_id, actor="STREAMLIT")
                        st.success("草稿已归档")
                        st.rerun()
                    except Exception as exc: st.exception(exc)
    with tabs[3]:
...
# -----------------------------------------------------------------------------
    ...
    with asset_tabs[3]:
        ...
        if not pdf_rows:
            ...
            pdf_cols = [column for column in [
                "pdf_id", "display_name", "company", "document_year", "size_bytes",
                "capture_reference_count", "path", "updated_at",
            ] if column in pdf_df.columns]
            manage_pdf = pdf_df[pdf_cols].copy()
            manage_pdf.insert(0, "选择", False)
            edited_pdf = st.data_editor(
                manage_pdf,
                use_container_width=True,
                hide_index=True,
                disabled=[column for column in manage_pdf.columns if column != "选择"],
                key="v613_pdf_asset_editor",
            )
            selected_pdf_ids = edited_pdf.loc[
                edited_pdf["选择"] == True, "pdf_id"
            ].astype(str).tolist()
            st.caption(
                "PDF 是源证据；这里只允许回收零引用文件。任何 Capture、Discovery、Occurrence "
                "或认证链引用都会阻止操作，系统不会级联删除机器证据。"
            )
            if selected_pdf_ids:
                if len(selected_pdf_ids) != 1:
                    st.info("PDF 生命周期操作一次只处理一个源文件，请只选择一行。")
                impact_rows = []
                total_references = 0
                for selected_pdf_id in selected_pdf_ids:
                    impact = BACKEND.pdf_service.dependency_impact(selected_pdf_id)
                    total_references += int(impact["reference_count"])
                    impact_rows.extend([
                        {"pdf_id": selected_pdf_id, **reference}
                        for reference in impact["references"]
                    ])
                if impact_rows:
                    st.warning(f"所选 PDF 共存在 {total_references} 条 Registry 引用，不能回收。")
                    st.dataframe(pd.DataFrame(impact_rows), use_container_width=True, hide_index=True)
                else:
                    st.success("所选 PDF 没有 Registry 引用，可以移入 PDF 回收站。")
                trash_pdf_ok = st.checkbox(
                    "确认将所选零引用 PDF 移入回收站",
                    key="v613_pdf_trash_confirm",
                )
                if st.button(
                    "移入 PDF 回收站",
                    disabled=(
                        not trash_pdf_ok or total_references > 0
                        or len(selected_pdf_ids) != 1
                    ),
                    key="v613_pdf_trash_button",
                    use_container_width=True,
                ):
                    try:
                        BACKEND.pdf_service.trash(selected_pdf_ids[0])
                    except Exception as exc:
                        st.error(f"PDF 回收失败：{type(exc).__name__}: {exc}")
                    else:
                        st.success("已回收 1 个 PDF；可在统一回收站恢复。")
                        st.rerun()

    # ------------------------------------------------------------------ Recycle bin
    with asset_tabs[4]:
        st.subheader("统一回收站")
        rt1, rt2, rt3, rt4 = st.tabs(["Capture Trash", "Batch Trash", "Merge Trash", "PDF Trash"])
        with rt1:
        ...
        with rt3:
            ...
            if not trash_merges:
                ...
                if c2.button("永久删除所选 Merge", disabled=not mids or not ok, use_container_width=True, key="v61_trash_merge_purge"):
                    ...
                    st.rerun()
        with rt4:
            trashed_pdfs = BACKEND.pdf_service.list(limit=10000, only_trash=True)
            if not trashed_pdfs:
                st.caption("PDF 回收站为空。")
            else:
                trashed_pdf_df = pd.DataFrame(trashed_pdfs)
                trash_pdf_cols = [column for column in [
                    "pdf_id", "display_name", "company", "document_year", "size_bytes",
                    "trash_path", "trashed_at",
                ] if column in trashed_pdf_df.columns]
                selected_pdf_id = st.selectbox(
                    "选择回收站 PDF",
                    trashed_pdf_df["pdf_id"].astype(str).tolist(),
                    format_func=lambda pdf_id: next(
                        (
                            f"{row.get('display_name') or row.get('filename')} · {pdf_id}"
                            for row in trashed_pdfs if str(row.get("pdf_id")) == str(pdf_id)
                        ),
                        str(pdf_id),
                    ),
                    key="v613_pdf_trash_selector",
                )
                st.dataframe(
                    trashed_pdf_df.loc[
                        trashed_pdf_df["pdf_id"].astype(str) == str(selected_pdf_id),
                        trash_pdf_cols,
                    ],
                    use_container_width=True,
                    hide_index=True,
                )
                pdf_restore_col, pdf_purge_col = st.columns(2)
                if pdf_restore_col.button(
                    "恢复 PDF", use_container_width=True, key="v613_pdf_restore_button",
                ):
                    try:
                        BACKEND.pdf_service.restore(selected_pdf_id)
                    except Exception as exc:
                        st.error(f"PDF 恢复失败：{type(exc).__name__}: {exc}")
                    else:
                        st.success("PDF 已恢复到原上传位置。")
                        st.rerun()
                purge_token = f"DELETE {selected_pdf_id}"
                purge_confirmation = pdf_purge_col.text_input(
                    f"永久删除请输入：{purge_token}",
                    key="v613_pdf_purge_confirmation",
                )
                if pdf_purge_col.button(
                    "永久删除 PDF",
                    disabled=purge_confirmation != purge_token,
                    use_container_width=True,
                    key="v613_pdf_purge_button",
                ):
                    try:
                        BACKEND.pdf_service.purge(
                            selected_pdf_id, confirmation=purge_confirmation,
                        )
                    except Exception as exc:
                        st.error(f"PDF 永久删除失败：{type(exc).__name__}: {exc}")
                    else:
                        st.success("PDF 文件、Registry 索引及其 SHA 缓存已永久删除。")
                        st.rerun()
