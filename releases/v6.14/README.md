...
> 状态：`DEVELOPMENT_CANDIDATE / NOT_PRODUCTION_RELEASE_CERTIFIED`。金融投资主表证据 V2 仍受 V1/V2 Shadow 门禁约束；浏览器 E2E 不在本轮范围。

> 状态：`DEVELOPMENT_CANDIDATE` · `NOT_PRODUCTION_RELEASE_CERTIFIED`

AXA Research 是面向保险公司年报的 PDF 表格发现、认证抓取、规范化与研究合表工作台。v6.14 在冻结的 v6.13 功能参照上开发金融投资主表证据 V2，并延续原生文本优先定位、候选页条件 OCR 与可复用页级 OCR 缓存。

本目录是隔离开发候选，尚不可替代已冻结的 v6.12.1 公开预发行资产，也不得直接上传为
新的 GitHub Release。真实数据链路与用户要求跳过的 E2E 不会被推断为通过。
...
- 发行状态：`DEVELOPMENT_CANDIDATE / NOT_PRODUCTION_RELEASE_CERTIFIED`
...
## v6.13 Registry 扩展（开发候选）

- 内置 `consolidated_balance_sheet` 与 `cash_flow_statement`：它们定位和认证合并主表整表，
  不把主表伪装为附注，也不在首版拆出重叠的现金流分区资产。
- “研究定义与表族 Registry”页面提供用户 Bundle 草稿、校验和显式启用；只有 ACTIVE
  Definition 才会进入批次和 Discovery 下拉。
- 内置金融投资、投资组合以及主表 Registry 在 service 层只读。用户定义仍走同一
  `Generic Discovery → CertifiedChildTableLink → Whole-table Capture` 正式链路。
- 合同详见 [docs/registry_user_contract.md](docs/registry_user_contract.md) 与
  [docs/ADR-010-main-statement-registry-and-user-overlay.md](docs/ADR-010-main-statement-registry-and-user-overlay.md)。
