...
def _row_period_key(row: dict[str, Any]) -> str:
    """Return the physical-row identity used by period classification.

    A canonical member may occur more than once on a transition statement.
    Period ownership therefore belongs to the source row, not to
    ``member_table``.  The fallback remains deterministic for compatibility
    fixtures that predate ``source_row_id``.
    """
    return str(
        row.get("source_row_id")
        or f"{row.get('member_table') or ''}::{row.get('_line_index', -1)}::"
        f"{normalize_text(str(row.get('raw_member_label') or ''))}"
    )


def _aggregate_member_period_statuses(
    rows: list[dict[str, Any]],
    row_period_statuses: dict[str, str],
) -> dict[str, str]:
    """Provide the legacy member-level summary without erasing row truth."""
    priority = {
        "ACTIVE_CURRENT_PERIOD": 0,
        "ACTIVE_COMPARATIVE_PERIOD": 1,
        "COMPARATIVE_ONLY_LEGACY_MEMBER": 2,
        "INACTIVE_CURRENT_PERIOD": 3,
        UNRESOLVED: 4,
        OUTSIDE_FAMILY: 5,
    }
    ...
    for row in rows:
        ...
        if not member_id:
            continue
        status = row_period_statuses.get(_row_period_key(row), UNRESOLVED)
        existing = aggregated.get(member_id)
        if existing is None or priority.get(status, 99) < priority.get(existing, 99):
            aggregated[member_id] = status
    return aggregated
...
class PingAnRowParser:
    def parse(self, lines: list[str], aliases: list[tuple[str, dict[str, Any]]], company: str) -> list[dict[str, Any]]:
        ...
        for index, line in enumerate(lines):
            ...
            note = compose_note_reference(header, raw_note) if raw_note else {
                "note_reference_raw": "", "note_reference_normalized": "",
                "note_reference_section": "", "note_reference_item": "",
                "note_reference_status": "NOTE_REFERENCE_UNRESOLVED",
            }
            key = f"{member['member_id']}|{note['note_reference_normalized'] or index}"
            if key in seen:
                continue
            seen.add(key)
        ...
        for index, line in enumerate(lines):
            ...
            if not matches:
                matches = [
                    (alias, member) for alias, member in aliases
                    if alias and (
                        (alias in joined_prev and alias not in normalize_text(prev_line))
            ...
            # Extract the ordered value lanes, including a standalone dash.
            # CPIC transition statements use ``-`` in the current column and
            # a numeric comparative value on the same physical row.  Ignoring
            # the dash silently re-labels the comparative value as current.
            amounts = []
            ...
            output.append({
                ...
                "period_resolution_status": (
                    "CURRENT_PERIOD_NOT_APPLICABLE"
                    if amount in {"-", "–", "—", "－"}
                    else "CURRENT_PERIOD_CONFIRMED" if amount else "UNSURE"
                ),
...
class StatementFamilyResolver:
    ...
    def resolve(self, *, index, family: dict[str, Any], members: list[dict[str, Any]],
        ...
        for rec in index:
            ...
            # Scope is a source fact.  Do not infer it from the order in which
            # formal statements appear in the filing.
            ...
            if source_scope == "UNKNOWN" and fallback_meta:
                ...
                if inferred_scope in {"CONSOLIDATED", "PARENT_COMPANY", "COMBINED_CONSOLIDATED_AND_PARENT"}:
                    source_scope = inferred_scope
        ...
        # Keep all page groups for the explicit, source-aware ranker.  The
        # former note-count tie-break could silently prefer a parent-company
        # page merely because it exposed more note references.
    ...
    def resolve_discovered_rows(
        ...
        *,
        ...
        for row in rows:
            ...
            scope = str(row.get("source_statement_scope") or row.get("scope") or "UNKNOWN")
        ...
        for (page, source_scope), source_rows in grouped.items():
            ...
            candidates.extend(page_candidates)
            # The discovered-row compatibility API returns both candidates and
            # resolutions.  Keep both outputs so valid rows do not disappear
            # before the resolver's coverage and parent semantics are applied.
            resolutions.extend(page_resolutions)
    ...
    def _resolve_page_rows(
        ...
        *,
        ...
        for row in selected:
            ...
            row_period_statuses[_row_period_key(row)] = status

        # A transition statement may expose the same canonical member twice:
        # one comparative-only pre-transition row and one current row.  Once
        # the parser has preserved the current-lane dash, the active sibling
        # is sufficient evidence that the dashed row is comparative-only.
        rows_by_member: dict[str, list[dict[str, Any]]] = {}
        for row in selected:
            rows_by_member.setdefault(str(row.get("member_table") or ""), []).append(row)
        for member_rows in rows_by_member.values():
            if len(member_rows) < 2:
                continue
            has_active_current = any(
                row_period_statuses.get(_row_period_key(row)) == "ACTIVE_CURRENT_PERIOD"
                for row in member_rows
            )
            if not has_active_current:
                continue
            for row in member_rows:
                key = _row_period_key(row)
                if (
                    row_period_statuses.get(key) in {UNRESOLVED, OUTSIDE_FAMILY}
                    and str(row.get("statement_amount_raw") or "").strip()
                    in {"", "-", "–", "—", "－"}
                ):
                    row_period_statuses[key] = "COMPARATIVE_ONLY_LEGACY_MEMBER"
        for row in outside_rows:
            row_period_statuses[_row_period_key(row)] = OUTSIDE_FAMILY

        period_statuses = _aggregate_member_period_statuses(
            [*selected, *outside_rows], row_period_statuses,
        )

        rows_with_status = [
            {
                **row,
                "member_period_status": row_period_statuses.get(
                    _row_period_key(row), UNRESOLVED,
                ),
            }
        ...
        selected_inside = [
            ...
            if row_period_statuses.get(_row_period_key(row)) != OUTSIDE_FAMILY
        ]
        evidence = {
            ...
            "row_period_statuses": row_period_statuses,
            ...
            "comparative_only_members": [
                ...
                for row in selected
                if row_period_statuses.get(_row_period_key(row))
                == "COMPARATIVE_ONLY_LEGACY_MEMBER"
            ],
        ...
        for sequence, row in enumerate(selected_inside, 1):
            ...
            period_status = row_period_statuses.get(
                _row_period_key(row), UNRESOLVED,
            )
            ocr_used = bool(row.get("ocr_used"))
            ...
            candidates.append({
                ...
                "statement_type": "BALANCE_SHEET", "scope": source_scope,
                ...
                "required_current_members": list(expected.get("required_current_members") or []),
                "optional_current_members": list(expected.get("optional_current_members") or []),
                "historical_variant_members": list(
                    (contract.get("expected_member_contracts") or {})
                    .get(regime, {})
                    .get(wanted_scope, {})
                    .get("historical_variant_members", [])
                ),
                "comparative_only_members": list(expected.get("comparative_only_members") or []),
                "family_total_status": "NOT_REPORTED",
                "statement_amount_raw": raw_amount,
    ...
    def _find_parent(text: str, aliases: list[str]) -> dict[str, str] | None:
        normalized_aliases = {normalize_text(a).strip("：:") for a in aliases}
        lines = [line.strip() for line in str(text or "").splitlines() if line.strip()]
        for index, line in enumerate(lines):
            compact = normalize_text(line).strip("：:")
            compact_base = re.sub(r"[：:\s0-9,，.、-]+$", "", compact)
            if compact in normalized_aliases or compact_base in normalized_aliases:
