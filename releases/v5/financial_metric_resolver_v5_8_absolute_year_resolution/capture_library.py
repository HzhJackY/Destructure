...
def capture_readiness(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "merge_ready": boundary_ready and header_ready,
...
def ensure_capture_metadata(
    ...
    data["merge_ready"] = readiness["merge_ready"]
    data["merge_blockers"] = readiness["merge_blockers"]
...
def capture_merge_ready(run_dir: Path) -> bool:
    result = load_capture_result(run_dir)
    return bool(capture_readiness(result)["merge_ready"])
