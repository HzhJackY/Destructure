...
def ensure_capture_metadata(
    ...
    data["boundary_status"] = derive_boundary_status(result)
...
def apply_boundary_review(
    ...
    metadata["row_count_official"] = int(official_long["row_order"].nunique())
...
def capture_merge_ready(run_dir: Path) -> bool:
    result = load_capture_result(run_dir)
    return derive_boundary_status(result) in MERGE_READY_STATUSES
...
def capture_record(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        **meta,
        ...
        "merge_ready": derive_boundary_status(result) in MERGE_READY_STATUSES,
    }
