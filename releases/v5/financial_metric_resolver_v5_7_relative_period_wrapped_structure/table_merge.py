...
def _resolve_relative_period_years(
    ...
    """
    Resolve relative financial-report periods at merge time.

    Capture evidence remains faithful:
      本年累计数 / 上年累计数

    When document_year is known:
      本年累计数 -> document_year
      上年累计数 -> document_year - 1

    The original period wording is preserved in source_period_label.
    """
    out = df.copy()
    if "year" not in out.columns:
        return out

    doc = str(document_year or "").strip()
    if not (len(doc) == 4 and doc.isdigit()):
        return out

    current_year = doc
    prior_year = str(int(doc) - 1)
    current_tokens = {
        "本年累计数", "本期累计数", "本期数", "本年数",
        "本期", "本年", "当期累计数", "当期",
        "期末", "年末", "本期期末", "本年末",
...
def load_capture_long(capture_dir: Path, metadata: dict[str, Any], table_id: str) -> pd.DataFrame:
    ...
    df = _resolve_relative_period_years(
        df,
