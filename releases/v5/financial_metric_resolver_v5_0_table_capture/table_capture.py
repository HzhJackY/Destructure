...
v5.0 named-table capture engine.
...
def locate_named_table(
    ...
    if start_page_override:
        ...
        for i, text in enumerate(texts, start=1):
            ...
            if score:
                title = table_query
                m2 = _NOTE_RE.search(text)
                if m2 and (not note_number or m2.group(1) == note_number):
                    title = f"{m2.group(1)}. {clean_cell(m2.group(2))}"
...
def capture_to_wide_df(result: TableCaptureResult) -> pd.DataFrame:
    ...
    original item. Section-header-only rows remain available in raw_long.
    """
    ...
    numeric["value_column"] = numeric.apply(col_name, axis=1)

    # Pivot ONLY on stable row identity. Using multiple metadata fields directly
    # as pivot_table index with dropna=False can create a Cartesian product.
...
def generate_capture_markdown(result: TableCaptureResult) -> str:
    ...
    lines += [
        ...
        "- `canonical_item` v5.0 默认不强制映射；跨公司细项统一应在后续taxonomy/人工裁决层完成。",
    ]
    return "\n".join(lines)
