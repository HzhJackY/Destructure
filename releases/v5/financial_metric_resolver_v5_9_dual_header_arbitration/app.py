...
# -*- coding: utf-8 -*-
"""
app.py — Financial Metric Resolver v5.9 GUI
...
# -----------------------------------------------------------------------------
    ...
    st.title("整表抓取")
    st.caption(
        "v5.9 双表头专家：Classic 处理传统年份，Generalized 保留 v5.7 复杂期间能力；"
    ...
    with st.expander("v5.9 双表头算法与边界裁决契约", expanded=False):
        st.code(
            """表头算法层
...
被截断的后续数据保存在 boundary_excluded_rows.csv。""",
    ...
    if not records:
        ...
        tabs = st.tabs([
            "预览", "边界复核", "表头算法裁决", "列拓扑复核",
            "表头维度复核", "合计/小计复核", "管理", "下载"
        ])
        ...
        with tabs[0]:
            ...
            m2.metric("表头维度", rec.get("header_dimension_status"))
            m3.metric("页码", f"{rec.get('start_page')}–{rec.get('end_page')}")
            m4.metric("正式行数", rec.get("row_count_official") or 0)
            m5.metric("可进入合表", "YES" if rec.get("merge_ready") else "NO")
        ...
        with tabs[2]:
            header_parser_arbitration_widget(hist, key_prefix=f"hist_{hist.name}")

        with tabs[3]:
            column_topology_review_widget(hist, key_prefix=f"hist_{hist.name}")

        with tabs[4]:
            header_dimension_review_widget(hist, key_prefix=f"hist_{hist.name}")

        with tabs[5]:
            path = hist / "table_reconciliation_audit.csv"
            audit = pd.read_csv(path) if path.exists() else pd.DataFrame()
            st.caption("Warning-only；系统不会因为算术不一致自动修改或删除数据。")
            st.dataframe(audit, use_container_width=True, hide_index=True)
        ...
        with tabs[7]:
            for label, filename, mime in [
                ...
                ("合计小计复核", "table_reconciliation_audit.csv", "text/csv"),
                ("表头算法候选", "header_parser_candidates.csv", "text/csv"),
                ("表头裁决JSON", "machine_header_arbitration.json", "application/json"),
                ("列拓扑复核JSON", "column_topology_review.json", "application/json"),
                ("Excel", "table_capture.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
            ]:
                path = hist / filename
                if path.exists():
    ...
    if blocked_records:
        with st.expander(f"有 {len(blocked_records)} 个整表抓取尚未通过结构认证，暂不可合表", expanded=False):
            ...
            st.caption(
                "请回到“整表抓取 → Capture Library”，依次查看“表头算法裁决 / 列拓扑复核 / 表头维度复核 / 边界复核”。"
            )
...
# -----------------------------------------------------------------------------
    ...
    st.title("数据管理 / 历史版本迁移")
    st.caption(
        "v5.6 起代码版本与数据资产分离。PDF、Capture、人工复核、Taxonomy 等长期资产存放在共享 DATA_HOME；"
        "升级程序版本时无需再复制整个 workspace。"
    )
