...
for p in [
    ...
    CACHE_DIR, BATCH_DIR, TABLE_CAPTURE_DIR, MERGE_DIR,
]:
...
# -----------------------------------------------------------------------------
    ...
    st.caption(
        ...
        "v5.1 使用空间ROI/列锚点重建；不强制统一细项语义：raw_item 永久保留，canonical_item 默认留待后续taxonomy/人工裁决。"
    )
    ...
    with st.expander("v5.1 空间整表抓取输出契约", expanded=False):
        st.code(
            """table_raw_long.csv
- 一行 = 一个原始明细 × 一个表头列
...
- 供后续跨公司taxonomy和人工映射使用""",
    ...
    if run_capture:
        ...
        try:
            ...
            with tabs[2]:
                ...
                st.caption(
                    "整表抓取层仅做确定性文本规范化；跨公司同义细项请进入“合表”工作区确认Taxonomy映射。"
                )
    ...
    capture_runs = sorted(
        [
            p for p in TABLE_CAPTURE_DIR.iterdir()
            if p.is_dir()
            and (p / "table_capture_result.json").exists()
            and (p / "table_raw_long.csv").exists()
        ],
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    ...
    if not capture_runs:
        st.info("还没有可用于合表的整表抓取结果。请先运行“整表抓取”。")
        st.stop()

    run_names = [p.name for p in capture_runs]
    selected_names = st.multiselect(
        "选择要合并的整表抓取运行",
        run_names,
        default=[],
        help="建议选择同一经济含义的附注表，例如多家公司/多年份的“业务及管理费和其他业务成本”。",
    )
    selected_dirs = [p for p in capture_runs if p.name in selected_names]
    ...
    merge_names = [p.name for p in merge_runs]
    default_merge = 0
    if st.session_state.active_merge_dir:
        active_name = Path(st.session_state.active_merge_dir).name
        if active_name in merge_names:
            default_merge = merge_names.index(active_name)

    merge_name = st.selectbox(
        "选择合表项目",
        merge_names,
        index=default_merge,
        key="merge_project_selector",
    )
    merge_dir = next(p for p in merge_runs if p.name == merge_name)
    st.session_state.active_merge_dir = str(merge_dir)
