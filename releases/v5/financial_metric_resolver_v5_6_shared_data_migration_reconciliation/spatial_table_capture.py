...
def infer_note_number_from_located_title(
    ...
    if q and q not in _line_compact(title) and _line_compact(title) not in q:
        return None
    return no
...
def _title_score(line_text: str, table_query: str, note_number: Optional[str]) -> int:
    ...
    if q and q in compact:
        score += 20
    matched = _match_note_heading(line_text, note_number) if note_number else _match_note_heading(line_text)
    if note_number and matched:
        score += 15
        if q and q in _line_compact(matched[1]):
            score += 15
    elif not note_number and matched and q and q in _line_compact(matched[1]):
        # A numbered heading matching the table name is stronger than prose/TOC text.
        score += 10
    return score
...
def _header_metadata(
    ...
    # Explicit per-column labels below the year row remain supported.
