...
# -*- coding: utf-8 -*-
"""
app.py — Financial Metric Resolver v6.2 transitional Streamlit UI
...
st.sidebar.caption(
    f"当前版本：v6.2 · 端口：{active_instance.get('port', 'direct')} · "
...
page = st.sidebar.radio(
    ...
    [
        ...
        "批量项目", "整表批量工作台", "整表抓取", "合表", "人工复核", "报告与审计", "数据资产管理", "系统与迁移",
    ],
)
...
# -----------------------------------------------------------------------------
    ...
    st.title("整表批量工作台")
    st.caption("v6.2：多 PDF 并行抓取、表族抓取、持久化作业与失败重试。每个 PDF × 目标表都是独立审计作业；失败不会中断同批其他任务。")
    pdfs = sorted(UPLOAD_DIR.glob("*.pdf"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not pdfs:
        st.info("请先在“PDF 项目”导入至少一份 PDF。")
        st.stop()
    selected_pdfs = st.multiselect("选择 PDF（可多选）", pdfs, default=pdfs[:1], format_func=lambda p: display_pdf_name(p.name), key="v62_batch_pdfs")
