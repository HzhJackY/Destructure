...
# -*- coding: utf-8 -*-
"""
app.py — Financial Metric Resolver v6.0.1 GUI
...
def request_runtime_action(action: str) -> tuple[bool, str]:
    ...
    if not token:
        return False, "当前程序不是通过 v6.0.1 single-instance launcher 启动，无法安全远程控制进程。"
    payload = {
        "action": str(action),
...
st.sidebar.caption(
    f"当前版本：v6.0.1 · 端口：{active_instance.get('port', 'direct')} · "
