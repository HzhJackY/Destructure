...
# -*- coding: utf-8 -*-
"""
app.py — Financial Metric Resolver v6.4 transitional Streamlit UI
...
page = st.sidebar.radio(
    ...
    [
        ...
        "批量项目", "整表批量工作台", "发现结果审核", "发现规则与学习库", "整表抓取", "合表", "人工复核", "报告与审计", "数据资产管理", "系统与迁移",
    ],
)
...
# -----------------------------------------------------------------------------
    ...
    st.subheader("通用主表引导发现（先预览，再确认抓取）")
    display_name = st.text_input("研究目标 / display_name", placeholder="例如：金融投资、保险合同负债、投资收益")
    preset_options = ["（无预设，纯通用发现）"] + list(__import__("generic_discovery").PRESETS)
    preset_name = st.selectbox("可选知识包", preset_options, help="预设只提供候选和历史别名，不是功能开关。")
    if st.button("发现并生成审核候选", disabled=not selected_pdfs or not display_name.strip(), key="v64_discover"):
        discovered = []
        for pdf in selected_pdfs:
            dim = __import__("pdf_selection_workspace").infer_pdf_dimensions(pdf)
            discovered.extend(BACKEND.discovery_service.preview(pdf, display_name=display_name.strip(), company=dim["company"], report_year=dim["year"], preset_name=None if preset_name.startswith("（") else preset_name))
        st.session_state["v64_discovery_preview"] = discovered
    preview = st.session_state.get("v64_discovery_preview", [])
    if preview:
        st.info(f"已保存 {len(preview)} 条不可变机器发现候选；请在“发现结果审核”确认后再执行细表抓取。")
        st.dataframe(pd.DataFrame(preview)[[c for c in ["company","report_year","statement_type","statement_item","note_reference","member_table","note_page","locator_method","confidence"] if c in pd.DataFrame(preview)]], use_container_width=True, hide_index=True)
    ...
    if st.button("创建并启动批量抓取", type="primary", disabled=not selected_pdfs or family is None, use_container_width=True):
        jobs = BACKEND.table_capture_runner.enqueue(pdf_paths=selected_pdfs, family=family, batch_id=batch_id.strip() or new_batch_id(), options={"note_number": note_number.strip() or None, "max_pages": int(max_pages), "header_parser_mode": parser})
        BACKEND.table_capture_runner.start(batch_id=jobs[0]["batch_id"], max_workers=int(workers))
        st.success(f"已创建 {len(jobs)} 个独立作业，正在后台以 {int(workers)} 个 Worker 执行。")
    st.divider()
    batch_options = sorted({j.get("batch_id") for j in BACKEND.job_service.list(limit=100000) if j.get("batch_id")})
    monitor_batch = st.selectbox("监控批次", batch_options, index=(batch_options.index(batch_id) if batch_id in batch_options else 0) if batch_options else None, key="v62_monitor_batch") if batch_options else None
    if monitor_batch:
        summary = BACKEND.table_capture_runner.monitor(monitor_batch)
        a, b, c, d = st.columns(4)
        a.metric("总作业", summary["total"]); b.metric("已完成", summary["complete"]); c.metric("运行中", summary["counts"].get("RUNNING", 0)); d.metric("失败", summary["counts"].get("FAILED", 0))
    ...
    st.title("发现结果审核")
    rows = BACKEND.discovery_registry.list_machine(limit=300)
    if not rows:
        st.info("暂无候选。请先在“整表批量工作台”运行通用发现。")
    else:
        table = pd.DataFrame(rows)
        st.dataframe(table[[c for c in ["discovery_id","company","report_year","display_name","statement_type","statement_item","note_reference","member_table","note_page","locator_method","confidence","status"] if c in table]], use_container_width=True, hide_index=True)
        selected = st.selectbox("选择候选", table["discovery_id"].tolist(), format_func=lambda x: f"{x[:14]} · {table.loc[table.discovery_id==x,'member_table'].iloc[0]}")
        label = st.radio("审核动作", ["ACCEPTED", "REJECTED", "OVERRIDDEN", "UNRESOLVED"], horizontal=True)
        reason = st.text_input("审核理由（建议填写）")
        scope = st.selectbox("适用范围", ["REPORT_ONLY", "COMPANY_STATEMENT", "GLOBAL_CANDIDATE"], index=1)
        override_name = st.text_input("修改后的 member_table（仅 OVERRIDDEN）")
        if st.button("保存审核决定", type="primary"):
            outcome = BACKEND.discovery_service.adjudicate(selected, label=label, reason=reason, scope=scope, override={"member_table": override_name} if override_name else {})
            st.success(f"已保存：{outcome['label']}；机器证据保持不变。")
