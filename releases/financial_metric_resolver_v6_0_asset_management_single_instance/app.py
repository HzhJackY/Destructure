...
# -*- coding: utf-8 -*-
"""
app.py — Financial Metric Resolver v6.0 GUI
...
from asset_management import (  # noqa: E402
    ...
    list_capture_assets,
    batch_summaries,
    capture_dirs_for_batch,
    dependency_impact,
    invalidate_captures,
    reactivate_captures,
    trash_captures,
    restore_trashed_captures,
    purge_trashed_captures,
    list_merge_assets,
    trash_merges,
    restore_merges,
    purge_merges,
    pdf_asset_records,
    rerun_capture_assets,
    new_batch_id,
    refresh_merge_dependency_statuses,
...
def header_parser_arbitration_widget(run_dir: Path, key_prefix: str) -> None:
    ...
    if st.button(
        ...
        try:
            ...
            st.session_state.active_table_capture_dir=str(new_dir)
            st.success(
                f"已创建人工指定算法的新 Capture：{new_dir.name}。"
                "原 Capture 未被覆盖。"
            )
            st.rerun()
        except Exception as exc:
...
def header_dimension_review_widget(run_dir: Path, key_prefix: str) -> None:
    ...
    if status == "REVIEW_REQUIRED":
        st.error(
            "检测到重复期间列无法由 year/scope/restated 唯一区分。"
    ...
    if not machine_df.empty:
        show_cols = [c for c in [
            "ordinal","source_column_index","header_raw","year","scope","restated"
    ...
    for col in ["year","scope","header_raw"]:
        if col in edit_df:
    ...
    edit_cols=[c for c in [
        "ordinal","source_column_index","year","scope","restated","header_raw"
    ...
    st.caption(
        ...
        "本集团/本集团/本公司/本公司。修改只影响正式输出维度，机器原始抓取保持不变。"
    )
    note=st.text_input(
        "表头维度复核备注（可选）",
...
def request_runtime_action(action: str) -> tuple[bool, str]:
    ...
    if not token:
        return False, "当前程序不是通过 v6.0 single-instance launcher 启动，无法安全远程控制进程。"
    payload = {
        "action": str(action),
...
st.sidebar.caption(
    f"当前版本：v6.0 · 端口：{active_instance.get('port', 'direct')} · "
...
page = st.sidebar.radio(
    ...
    [
        ...
        "批量项目", "整表抓取", "合表", "人工复核", "报告与审计", "数据资产管理", "系统与迁移",
    ],
)
...
# -----------------------------------------------------------------------------
    ...
    st.title("整表抓取")
    st.caption(
        "v6.0 保留 v5.9 双表头专家，同时新增独立数据资产管理与批量生命周期。"
    ...
    if b2.button("生成新批次ID", use_container_width=True, key="new_table_capture_batch"):
        st.session_state.table_capture_batch_id = new_batch_id()
        st.rerun()

    with st.expander("v6.0 双表头算法与资产批次契约", expanded=False):
        st.code(
            """表头算法层
...
被截断的后续数据保存在 boundary_excluded_rows.csv。""",
    ...
    if run_capture:
        ...
        try:
            ...
            result.stats["capture_batch_id"] = effective_batch_id
            ...
            status.update(
                label=(
                    ...
                    f"{len(result.rows)} 行 · 边界={boundary_status} · 表头={header_status} · "
                    f"算法={(result.stats or {}).get('header_parser','legacy')}"
                ),
                state="complete",
                expanded=False,
            )
    ...
    if blocked_records:
        with st.expander(f"有 {len(blocked_records)} 个整表抓取尚未通过结构认证，暂不可合表", expanded=False):
            ...
            st.caption(
                "请到“数据资产管理 → Captures”选择对应记录，完成“表头算法裁决 / 列拓扑复核 / 表头维度复核 / 边界复核”。"
            )
...
# -----------------------------------------------------------------------------
    ...
    st.title("数据资产管理中心")
    st.caption(
        "统一管理 Capture / Batch / Merge / PDF / 回收站。批量抓错时优先“废除（INVALIDATED）”而不是直接删除；"
        "机器证据保留，失效 Capture 自动使依赖 Merge 标记为 STALE。"
    )
    ...
    # ------------------------------------------------------------------ Captures
    with asset_tabs[0]:
        records = list_capture_assets(TABLE_CAPTURE_DIR, include_trash=False)
        if not records:
            st.info("暂无 Capture 资产。")
        else:
            df = pd.DataFrame(records)
            st.subheader("Capture 批量管理")
            f1, f2, f3, f4, f5 = st.columns(5)
            lifecycle_options = ["全部"] + sorted({str(x) for x in df["lifecycle_status"].dropna().unique()})
            lifecycle_filter = f1.selectbox("生命周期", lifecycle_options, key="asset_cap_lifecycle")
            table_filter = f2.text_input("表名筛选", value="", key="asset_cap_table")
            company_filter = f3.text_input("公司筛选", value="", key="asset_cap_company")
            year_filter = f4.text_input("报告年", value="", key="asset_cap_year")
            version_options = ["全部"] + sorted({str(x) for x in df.get("producer_version", pd.Series(dtype=str)).dropna().unique()})
            version_filter = f5.selectbox("Producer Version", version_options, key="asset_cap_version")

            filtered = df.copy()
            if lifecycle_filter != "全部":
                filtered = filtered[filtered["lifecycle_status"].astype(str) == lifecycle_filter]
            if table_filter.strip():
                filtered = filtered[filtered["table_query"].fillna("").astype(str).str.contains(table_filter.strip(), case=False, regex=False)]
            if company_filter.strip():
                filtered = filtered[filtered.get("company", pd.Series(index=filtered.index, dtype=str)).fillna("").astype(str).str.contains(company_filter.strip(), case=False, regex=False)]
            if year_filter.strip():
                filtered = filtered[filtered.get("document_year", pd.Series(index=filtered.index, dtype=str)).fillna("").astype(str) == year_filter.strip()]
            if version_filter != "全部":
                filtered = filtered[filtered.get("producer_version", pd.Series(index=filtered.index, dtype=str)).fillna("").astype(str) == version_filter]

            st.caption(f"筛选结果：{len(filtered)} / {len(df)} 条")
            display_cols = [c for c in [
                "run_id", "lifecycle_status", "company", "document_year", "source_pdf_display",
                "table_query", "batch_id", "producer_version", "header_parser", "boundary_status",
                "header_dimension_status", "merge_ready", "created_at", "invalidation_reason_code",
            ] if c in filtered.columns]
            manage_df = filtered[display_cols].copy()
            manage_df.insert(0, "选择", False)
            edited = st.data_editor(
                manage_df,
                use_container_width=True,
                hide_index=True,
                disabled=[c for c in manage_df.columns if c != "选择"],
                key="asset_capture_editor",
                height=430,
            )
            select_all = st.checkbox(
                f"选择全部 {len(filtered)} 条筛选结果",
                key="asset_capture_select_all",
                help="类似 Gmail 的“全选筛选结果”；不只选择当前可见行。",
            )
            selected_ids = (
                set(filtered["run_id"].astype(str).tolist())
                if select_all
                else set(edited.loc[edited["选择"] == True, "run_id"].astype(str).tolist())
            )
            selected_records = [r for r in records if str(r.get("run_id")) in selected_ids]
            selected_dirs = [Path(r["run_dir"]) for r in selected_records]
            st.write(f"**已选择：{len(selected_dirs)} 条**")

            if selected_dirs:
                impact = dependency_impact(selected_ids, MERGE_DIR)
                if impact["dependent_merge_count"]:
                    st.warning(
                        f"这些 Capture 被 {impact['dependent_merge_count']} 个 Merge Project 引用。"
                        "废除/回收后，对应 Merge 会自动标记 STALE_SOURCE_INVALIDATED。"
                    )
                    with st.expander("查看受影响 Merge", expanded=False):
                        st.dataframe(pd.DataFrame(impact["dependent_merges"]), use_container_width=True, hide_index=True)

            op1, op2, op3, op4 = st.columns(4)
            with op1:
                reason_code = st.selectbox("废除原因", INVALIDATION_REASON_CODES, key="asset_invalidate_reason")
                reason_note = st.text_input("废除备注", key="asset_invalidate_note", placeholder="例如：v5.8 4列错误识别为8列")
                confirm_invalidate = st.checkbox("确认批量废除", key="asset_invalidate_confirm")
                if st.button(
                    "批量废除 INVALIDATED",
                    use_container_width=True,
                    type="primary",
                    disabled=not selected_dirs or not confirm_invalidate,
                    key="asset_bulk_invalidate",
                ):
                    out = invalidate_captures(
                        selected_dirs,
                        reason_code=reason_code,
                        note=reason_note,
                        merge_root=MERGE_DIR,
                    )
                    st.success(f"已废除 {len(out['invalidated'])} 条；影响 Merge {len(out['stale_merges'])} 个。")
                    st.rerun()
            with op2:
                confirm_reactivate = st.checkbox("确认恢复为 ACTIVE", key="asset_reactivate_confirm")
                if st.button(
                    "批量恢复 ACTIVE",
                    use_container_width=True,
                    disabled=not selected_dirs or not confirm_reactivate,
                    key="asset_bulk_reactivate",
                ):
                    out = reactivate_captures(selected_dirs)
                    refresh_merge_dependency_statuses(TABLE_CAPTURE_DIR, MERGE_DIR)
                    st.success(f"已恢复 {len(out['reactivated'])} 条 Capture。")
                    st.rerun()
            with op3:
                confirm_trash = st.checkbox("确认移入回收站", key="asset_trash_confirm")
                if st.button(
                    "批量移入回收站",
                    use_container_width=True,
                    disabled=not selected_dirs or not confirm_trash,
                    key="asset_bulk_trash",
                ):
                    out = trash_captures(selected_dirs, TABLE_CAPTURE_TRASH_DIR, merge_root=MERGE_DIR)
                    st.success(f"已移入回收站 {len(out['trashed'])} 条。")
                    st.rerun()
            with op4:
                rerun_parser = st.selectbox(
                    "重新抓取 Parser",
                    ["AUTO", "ABSOLUTE_YEAR_CLASSIC", "GENERALIZED_PERIOD_V57"],
                    key="asset_rerun_parser",
                )
                confirm_rerun = st.checkbox("确认从原PDF批量重抓", key="asset_rerun_confirm")
                if st.button(
                    "批量重新抓取",
                    use_container_width=True,
                    disabled=not selected_dirs or not confirm_rerun,
                    key="asset_bulk_rerun",
                ):
                    batch_id = new_batch_id("RERUN_BATCH")
                    prog = st.progress(0.0)
                    live = st.empty()
                    def rerun_progress(ev):
                        prog.progress(min(1.0, ev["index"] / max(1, ev["total"])))
                        live.write(f"{ev['index']}/{ev['total']} · {ev['source']} · {ev['status']}")
                    out = rerun_capture_assets(
                        selected_dirs,
                        capture_root=TABLE_CAPTURE_DIR,
                        upload_root=UPLOAD_DIR,
                        parser_mode=rerun_parser,
                        batch_id=batch_id,
                        progress_callback=rerun_progress,
                    )
                    st.success(f"新批次 {batch_id}：成功 {len(out['created'])}，失败 {len(out['failures'])}。")
                    if out["failures"]:
                        st.dataframe(pd.DataFrame(out["failures"]), use_container_width=True, hide_index=True)
                    st.rerun()

            st.divider()
            st.subheader("单条 Capture 预览 / 结构审核")
            detail_map = {f"{r.get('display_name') or r.get('run_id')} · {r.get('lifecycle_status')} · {r.get('run_id')}": r for r in records}
            detail_label = st.selectbox("选择 Capture", list(detail_map), key="asset_capture_detail")
            detail = detail_map[detail_label]
            hist = Path(detail["run_dir"])
            st.session_state.active_table_capture_dir = str(hist)
            dmeta = ensure_capture_metadata(hist)
            m1,m2,m3,m4,m5 = st.columns(5)
            m1.metric("生命周期", dmeta.get("lifecycle_status", "ACTIVE"))
            m2.metric("边界", detail.get("boundary_status"))
            m3.metric("表头", detail.get("header_dimension_status"))
            m4.metric("Batch", str(dmeta.get("batch_id") or "-")[:28])
            m5.metric("可合表", "YES" if detail.get("merge_ready") else "NO")
            detail_tabs = st.tabs(["预览", "边界复核", "表头算法裁决", "列拓扑复核", "表头维度复核", "合计/小计复核"])
            with detail_tabs[0]:
                pwide = hist / "table_raw_wide.csv"
                st.dataframe(pd.read_csv(pwide) if pwide.exists() else pd.DataFrame(), use_container_width=True, hide_index=True)
            ...
            with detail_tabs[1]:
                boundary_review_widget(hist, key_prefix=f"asset_{hist.name}")
            with detail_tabs[2]:
                header_parser_arbitration_widget(hist, key_prefix=f"asset_{hist.name}")
            with detail_tabs[3]:
                column_topology_review_widget(hist, key_prefix=f"asset_{hist.name}")
            with detail_tabs[4]:
                header_dimension_review_widget(hist, key_prefix=f"asset_{hist.name}")
            with detail_tabs[5]:
            ...
            if st.button("导出当前 Capture 资产清单 CSV", key="asset_export_inventory"):
                path = export_asset_inventory(records, ASSET_REPORT_DIR)
                st.success(f"已生成：{path}")

    # ------------------------------------------------------------------ Batches
    with asset_tabs[1]:
        records = list_capture_assets(TABLE_CAPTURE_DIR, include_trash=True)
        batches = batch_summaries(records)
        st.subheader("Capture Batches")
        if not batches:
            st.info("暂无 Batch。")
        else:
            bdf = pd.DataFrame(batches)
            bdf.insert(0, "选择", False)
            bedit = st.data_editor(
                bdf,
                use_container_width=True,
                hide_index=True,
                disabled=[c for c in bdf.columns if c != "选择"],
                key="asset_batch_editor",
            )
            selected_batches = bedit.loc[bedit["选择"] == True, "batch_id"].astype(str).tolist()
            selected_batch_dirs=[]
            for bid in selected_batches:
                selected_batch_dirs.extend(capture_dirs_for_batch(records, bid))
            st.caption(f"已选择 {len(selected_batches)} 个批次 / {len(selected_batch_dirs)} 个 Capture")
            b1,b2,b3 = st.columns(3)
            with b1:
                breason=st.selectbox("整批废除原因",INVALIDATION_REASON_CODES,key="batch_reason")
                bnote=st.text_input("整批废除备注",key="batch_note")
                bconfirm=st.checkbox("确认整批废除",key="batch_invalid_confirm")
                if st.button("废除所选批次",disabled=not selected_batch_dirs or not bconfirm,use_container_width=True,key="batch_invalidate"):
                    out=invalidate_captures(selected_batch_dirs,reason_code=breason,note=bnote,merge_root=MERGE_DIR)
                    st.success(f"已废除 {len(out['invalidated'])} 个 Capture。")
                    st.rerun()
            with b2:
                brerun=st.checkbox("确认整批重新抓取",key="batch_rerun_confirm")
                if st.button("重新运行所选批次",disabled=not selected_batch_dirs or not brerun,use_container_width=True,key="batch_rerun"):
                    new_bid=new_batch_id("BATCH_REPLACEMENT")
                    out=rerun_capture_assets(selected_batch_dirs,capture_root=TABLE_CAPTURE_DIR,upload_root=UPLOAD_DIR,parser_mode="AUTO",batch_id=new_bid)
                    st.success(f"替代批次 {new_bid}：成功 {len(out['created'])}，失败 {len(out['failures'])}。")
                    st.rerun()
            with b3:
                btrash=st.checkbox("确认整批回收",key="batch_trash_confirm")
                if st.button("所选批次移入回收站",disabled=not selected_batch_dirs or not btrash,use_container_width=True,key="batch_trash"):
                    out=trash_captures(selected_batch_dirs,TABLE_CAPTURE_TRASH_DIR,merge_root=MERGE_DIR)
    ...
    with asset_tabs[2]:
        ...
        if st.button("重新检查全部 Merge 依赖状态", key="refresh_merge_dependencies"):
            out=refresh_merge_dependency_statuses(TABLE_CAPTURE_DIR,MERGE_DIR)
            st.success(f"已检查 {len(out['updated'])} 个 Merge。")
            st.rerun()
        merges=list_merge_assets(MERGE_DIR,include_trash=False)
        if not merges:
            st.info("暂无 Merge Project。")
        else:
            mdf=pd.DataFrame(merges)
            cols=[c for c in ["run_id","display_name","table_id","source_count","dependency_status","stale_capture_run_ids","created_at"] if c in mdf.columns]
            manage=mdf[cols].copy();manage.insert(0,"选择",False)
            medit=st.data_editor(manage,use_container_width=True,hide_index=True,disabled=[c for c in manage.columns if c!="选择"],key="asset_merge_editor")
            mids=set(medit.loc[medit["选择"]==True,"run_id"].astype(str))
            mdirs=[Path(r["run_dir"]) for r in merges if str(r.get("run_id")) in mids]
            confirm=st.checkbox("确认将所选 Merge 移入回收站",key="asset_merge_trash_confirm")
            if st.button("批量回收 Merge",disabled=not mdirs or not confirm,key="asset_merge_trash",use_container_width=True):
                out=trash_merges(mdirs,MERGE_TRASH_DIR)
    ...
    with asset_tabs[3]:
        ...
        cap_records=list_capture_assets(TABLE_CAPTURE_DIR,include_trash=True)
        pdf_rows=pdf_asset_records(UPLOAD_DIR,cap_records)
        if not pdf_rows:
            st.info("暂无 PDF。")
        else:
            st.dataframe(pd.DataFrame(pdf_rows),use_container_width=True,hide_index=True)
            st.caption("PDF 是源证据。v6.0 默认不提供批量硬删除；被 Capture 引用的 PDF 应长期保留。")

    # ------------------------------------------------------------------ Recycle bin
    with asset_tabs[4]:
        st.subheader("统一回收站")
        rt1,rt2=st.tabs(["Capture Trash","Merge Trash"])
        with rt1:
            trash_records=[r for r in list_capture_assets(TABLE_CAPTURE_DIR,include_trash=True) if r.get("is_trashed")]
            if not trash_records:
                st.caption("Capture 回收站为空。")
            else:
                tdf=pd.DataFrame(trash_records)
                cols=[c for c in ["run_id","display_name","pre_trash_status","batch_id","source_pdf_display","table_query","trashed_at"] if c in tdf.columns]
                manage=tdf[cols].copy();manage.insert(0,"选择",False)
                tedit=st.data_editor(manage,use_container_width=True,hide_index=True,disabled=[c for c in manage.columns if c!="选择"],key="trash_capture_editor")
                tids=set(tedit.loc[tedit["选择"]==True,"run_id"].astype(str))
                tdirs=[Path(r["run_dir"]) for r in trash_records if str(r.get("run_id")) in tids]
                c1,c2=st.columns(2)
                if c1.button("批量恢复 Capture",disabled=not tdirs,use_container_width=True,key="trash_capture_restore"):
                    out=restore_trashed_captures(tdirs,TABLE_CAPTURE_DIR)
                    refresh_merge_dependency_statuses(TABLE_CAPTURE_DIR,MERGE_DIR)
                    st.success(f"已恢复 {len(out['restored'])} 条。")
                    st.rerun()
                purge_confirm=st.checkbox("确认永久删除所选 Capture（不可恢复）",key="trash_capture_purge_confirm")
                if c2.button("永久删除所选 Capture",disabled=not tdirs or not purge_confirm,use_container_width=True,key="trash_capture_purge"):
                    out=purge_trashed_captures(tdirs)
                    st.success(f"已永久删除 {len(out['purged'])} 条。")
                    st.rerun()
        with rt2:
            trash_merges_rows=[r for r in list_merge_assets(MERGE_DIR,include_trash=True) if r.get("is_trashed")]
            if not trash_merges_rows:
                st.caption("Merge 回收站为空。")
            else:
                tdf=pd.DataFrame(trash_merges_rows)
                cols=[c for c in ["run_id","display_name","table_id","source_count","created_at"] if c in tdf.columns]
                manage=tdf[cols].copy();manage.insert(0,"选择",False)
                tedit=st.data_editor(manage,use_container_width=True,hide_index=True,disabled=[c for c in manage.columns if c!="选择"],key="trash_merge_editor")
                tids=set(tedit.loc[tedit["选择"]==True,"run_id"].astype(str))
                tdirs=[Path(r["run_dir"]) for r in trash_merges_rows if str(r.get("run_id")) in tids]
                c1,c2=st.columns(2)
                if c1.button("批量恢复 Merge",disabled=not tdirs,use_container_width=True,key="trash_merge_restore"):
                    out=restore_merges(tdirs,MERGE_DIR)
                    st.success(f"已恢复 {len(out['restored'])} 个 Merge。")
                    st.rerun()
                purge_confirm=st.checkbox("确认永久删除所选 Merge（不可恢复）",key="trash_merge_purge_confirm")
                if c2.button("永久删除所选 Merge",disabled=not tdirs or not purge_confirm,use_container_width=True,key="trash_merge_purge"):
                    out=purge_merges(tdirs)
...
# -----------------------------------------------------------------------------
    ...
    st.title("系统与迁移")
    st.caption(
        "v6.0 延续共享 DATA_HOME，并新增数据资产生命周期、批量管理与 Single-Instance Launcher。"
