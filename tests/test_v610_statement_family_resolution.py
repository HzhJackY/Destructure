...
600"""
    ...
    assert resolution["family_total_status"] == "NOT_REPORTED"
    # v6.11 member-contract V2 moves the two research dependencies outside
    # the accounting statement family.  They remain auditable in the
    # resolution evidence but never become family candidates.
    assert len(rows) == 4
    ...
    assert set(resolution["outside_family_members"]) == {
        "time_deposits", "long_term_equity",
    }
    assert not {"time_deposits", "long_term_equity"} & set(
